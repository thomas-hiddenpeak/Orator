...
int main() {
  ...
  // are a numerical near-tie the bf16/mel ~1e-3 noise can flip, so exclude them;
  // the model forward is proven exact against the oracle in test_aligner_lm.
  // Every word the reference DID place must agree within one 80 ms bucket.
  int worst = 0, off = 0, checked = 0, skipped = 0;
  for (size_t k = 0; k < units.size(); ++k) {
    const int rs = word_ms[2 * k], re = word_ms[2 * k + 1];
    if (re <= rs) { ++skipped; continue; }  // degenerate reference word
  ...
  std::printf("  checked=%d skipped(degenerate)=%d worst diff=%d ms off>80ms=%d\n",

...
std::vector<BusinessSpeakerPipeline::Entry> SpeakerFusionPolicy::Apply(
  ...
  using TextSeg = BusinessSpeakerPipeline::TextSeg;
  ...
  struct AlignedSourceUnit {
    bool source_complete = false;
    int source_start = -1;
    int source_end = -1;
    double start = 0.0;
    double end = 0.0;
    bool zero_duration = false;
  };
  std::vector<CharacterTime> character_times(character_count);
  std::vector<int> zero_duration_alignment_count(character_count, 0);
  std::vector<AlignedSourceUnit> aligned_source_units;
  ...
  if (alignment != pipeline.align_.end()) {
    ...
    for (const auto& unit : alignment->second.units) {
      ...
      bool contiguous = !matched_indices.empty();
      for (std::size_t index = 1; index < matched_indices.size(); ++index) {
        if (matched_indices[index] != matched_indices[index - 1] + 1) {
          contiguous = false;
          break;
        }
      }
      const std::size_t unit_character_count = unit_offsets.size() - 1;
      const bool source_complete =
          contiguous && matched_indices.size() == unit_character_count;
      aligned_source_units.push_back(
          {source_complete,
           source_complete ? static_cast<int>(matched_indices.front()) : -1,
           source_complete ? static_cast<int>(matched_indices.back() + 1) : -1,
           unit.start, unit.end, NearEqual(unit.start, unit.end)});
      if (NearEqual(unit.start, unit.end)) {
        for (const std::size_t index : matched_indices) {
          ++zero_duration_alignment_count[index];
        }
      }
  ...
  auto has_three_run_middle_slot_handoff = [&](const auto& item,
    ...
    if (item.kind != "punctuation_phrase" ||
        ...
        !item.robust_gallery_complete || !selected.score_pass ||
        !selected.margin_pass) {
      return false;
    }

    auto has_unique_identities = [](const auto& scores) {
      std::set<std::string> identities;
      for (const auto& score : scores) {
        if (score.speaker_id.empty() ||
            !identities.insert(score.speaker_id).second) {
          return false;
        }
      }
      return identities.size() >= 2;
    };
    if (!has_unique_identities(item.session_scores) ||
        !has_unique_identities(item.robust_scores)) {
      return false;
    }
    const auto robust = rank(item.robust_scores, duration);
    if (!robust || robust->speaker_id != selected.speaker_id ||
        !robust->margin_pass || robust->score_pass) {
      return false;
    }
    ...
    if (runs.size() != 3 || runs[0].speaker_id != runs[2].speaker_id ||
        runs[0].speaker_id == selected.speaker_id ||
        runs[1].speaker_id != selected.speaker_id ||
        runs[1].source_end - runs[1].source_start != 1) {
      return false;
    }

    const int middle_index = runs[1].source_start;
    const std::string middle_codepoint =
        text.text.substr(offsets[middle_index],
                         offsets[middle_index + 1] - offsets[middle_index]);
    ...
    if (middle_codepoint == " " || middle_codepoint == "\t" ||
        middle_codepoint == "\n" || middle_codepoint == "\r" ||
        punctuation.count(middle_codepoint) != 0) {
      return false;
    }

    const int previous_index = middle_index - 1;
    const int following_index = middle_index + 1;
    auto has_primary_provenance = [&](int index) {
      return index >= item.source_start && index < item.source_end &&
             character_times[index].available &&
             (labels[index].base_reason == "primary_speaker_tie_break" ||
              labels[index].base_reason ==
                  "primary_speaker_overlap_refinement");
    };
    if (!has_primary_provenance(previous_index) ||
        !has_primary_provenance(middle_index) ||
        !has_primary_provenance(following_index)) {
      return false;
    }

    auto unique_primary_at = [&](int index) -> const SpeakerSeg* {
      const double midpoint =
          0.5 * (character_times[index].start + character_times[index].end);
      const SpeakerSeg* found = nullptr;
      for (const auto& primary : pipeline.primary_speakers_) {
        if (midpoint < primary.start - 1e-9 || midpoint >= primary.end - 1e-9) {
          continue;
        }
    ...
    const SpeakerSeg* previous_primary = unique_primary_at(previous_index);
    const SpeakerSeg* middle_primary = unique_primary_at(middle_index);
    const SpeakerSeg* following_primary = unique_primary_at(following_index);
    const double minimum =
        pipeline.config_.voiceprint_primary_consensus_min_sec;
    if (previous_primary == nullptr || middle_primary == nullptr ||
        following_primary == nullptr ||
        previous_primary->speaker_id != runs[0].speaker_id ||
        middle_primary->speaker_id != selected.speaker_id ||
        following_primary->speaker_id != runs[2].speaker_id ||
        previous_primary == middle_primary ||
        middle_primary == following_primary ||
        previous_primary->end > middle_primary->start + 1e-9 ||
        middle_primary->end > following_primary->start + 1e-9 ||
        middle_primary->end - middle_primary->start + 1e-9 < minimum ||
        identity_coverage(pipeline.speakers_, middle_primary->start,
                          middle_primary->end, selected.speaker_id) +
                1e-9 <
            middle_primary->end - middle_primary->start) {
      return false;
    }

    auto aligned_intervals = [&](const BaseRun& run) {
      std::vector<std::pair<double, double>> intervals;
      for (int index = run.source_start; index < run.source_end; ++index) {
        if (character_times[index].available) {
          intervals.push_back(
              {character_times[index].start, character_times[index].end});
        }
    ...
    auto aligned_identity_coverage = [&](const auto& intervals,
                                         const auto& segments,
                                         const std::string& speaker_id) {
      std::vector<std::pair<double, double>> intersections;
    ...
    for (const BaseRun* outer : {&runs[0], &runs[2]}) {
      const auto intervals = aligned_intervals(*outer);
      if (CoveredDuration(intervals) + 1e-9 < minimum ||
          aligned_identity_coverage(intervals, pipeline.speakers_,
                                    outer->speaker_id) +
                  1e-9 <
              minimum ||
          aligned_identity_coverage(intervals, pipeline.primary_speakers_,
                                    outer->speaker_id) +
                  1e-9 <
              minimum) {
        return false;
    ...
    std::vector<const SpeakerVoiceprintEvidence*> overlapping_vads;
    for (const auto& candidate : pipeline.voiceprint_vad_) {
      if (candidate.kind == "vad" && candidate.end > candidate.start &&
          Overlap(item.start, item.end, candidate.start, candidate.end) >
              1e-9) {
        overlapping_vads.push_back(&candidate);
      }
    }
    if (overlapping_vads.size() != 2) return false;
    std::sort(overlapping_vads.begin(), overlapping_vads.end(),
              [](const auto* left, const auto* right) {
    ...
    const auto* first_vad = overlapping_vads[0];
    const auto* second_vad = overlapping_vads[1];
    if (!first_vad->embedding_available ||
        !first_vad->robust_gallery_complete ||
        !second_vad->embedding_available ||
        !second_vad->robust_gallery_complete ||
        first_vad->end > second_vad->start + 1e-9 ||
        first_vad->start > item.start + 1e-9 ||
        item.start + 1e-9 >= first_vad->end ||
        second_vad->start + 1e-9 >= item.end ||
        second_vad->end + 1e-9 < item.end ||
        first_vad->end + 1e-9 >= item.end ||
        second_vad->start <= item.start + 1e-9) {
      return false;
    }

    auto vad_ranks = [&](const SpeakerVoiceprintEvidence& vad,
                         const std::string& expected) {
      if (!has_unique_identities(vad.session_scores) ||
          !has_unique_identities(vad.robust_scores)) {
        return false;
      }
      const double vad_duration = vad.end - vad.start;
      const auto session_rank = rank(vad.session_scores, vad_duration);
      const auto robust_rank = rank(vad.robust_scores, vad_duration);
      return session_rank && robust_rank && session_rank->margin_pass &&
             robust_rank->margin_pass && session_rank->speaker_id == expected &&
             robust_rank->speaker_id == expected;
    };
    return vad_ranks(*first_vad, runs[0].speaker_id) &&
           vad_ranks(*second_vad, selected.speaker_id);
  };
  ...
  auto source_leading_phrase_tail_isolation_challenge = [&](const auto& item)
      -> std::optional<std::string> {
    const double phrase_duration = item.end - item.start;
    if (item.kind != "punctuation_phrase" || item.text_id != text.id ||
    ...
    for (const auto* candidate : source_evidence) {
      if (candidate->text_id != item.text_id) continue;
      if (candidate->kind == "business_interval" &&
          candidate->source_start == item.source_start &&
          candidate->source_end == character_count &&
          NearEqual(candidate->start, item.start) &&
          candidate->end > item.end + 1e-9) {
        if (containing_interval != nullptr) return std::nullopt;
        containing_interval = candidate;
      } else if (candidate->kind == "complete_source" &&
                 candidate->source_start == 0 &&
                 candidate->source_end == character_count) {
        if (complete_source != nullptr) return std::nullopt;
        complete_source = candidate;
      }
    }
    if (containing_interval == nullptr || complete_source == nullptr ||
        !containing_interval->embedding_available ||
        !containing_interval->robust_gallery_complete ||
        !complete_source->embedding_available ||
        !complete_source->robust_gallery_complete ||
        containing_interval->source_end <= item.source_end ||
        complete_source->start > containing_interval->start + 1e-9 ||
        complete_source->end + 1e-9 < containing_interval->end) {
      return std::nullopt;
    }
    const double interval_duration =
        containing_interval->end - containing_interval->start;
    const double complete_duration =
        complete_source->end - complete_source->start;
    if (interval_duration + 1e-9 <
            pipeline.config_.voiceprint_primary_consensus_min_sec ||
        interval_duration + 1e-9 >=
            pipeline.config_.voiceprint_short_max_sec ||
        complete_duration + 1e-9 <
            pipeline.config_.voiceprint_short_max_sec) {
      return std::nullopt;
    }
    ...
    int visible_index = -1;
    int visible_count = 0;
    for (int index = item.source_end; index < character_count; ++index) {
      const std::string codepoint = text.text.substr(
          offsets[index], offsets[index + 1] - offsets[index]);
      if (codepoint == " " || codepoint == "\t" || codepoint == "\n" ||
          codepoint == "\r" || punctuation.count(codepoint) != 0) {
        continue;
      }
      visible_index = index;
      ++visible_count;
    }
    const std::string final_codepoint = text.text.substr(
        offsets[character_count - 1],
        offsets[character_count] - offsets[character_count - 1]);
    if (visible_count != 1 || visible_index != item.source_end ||
        punctuation.count(final_codepoint) == 0) {
      return std::nullopt;
    }

    const SpeakerVoiceprintEvidence* tail_unit = nullptr;
    for (const auto* candidate : source_evidence) {
      if (candidate->kind != "aligned_unit" ||
          candidate->text_id != item.text_id ||
          candidate->source_end <= item.source_end ||
          candidate->source_start >= character_count ||
          candidate->end <= candidate->start) {
        continue;
      }
      if (tail_unit != nullptr || candidate->source_start != visible_index ||
          candidate->source_end != visible_index + 1) {
        return std::nullopt;
      }
      tail_unit = candidate;
    }
    if (tail_unit == nullptr ||
        tail_unit->start - item.end + 1e-9 <
            pipeline.config_.align_snap_pause_sec ||
        !NearEqual(tail_unit->end, containing_interval->end)) {
      return std::nullopt;
    }

    auto unique_covering_segment = [](const std::vector<SpeakerSeg>& segments,
                                      double start, double end) {
      const SpeakerSeg* covering = nullptr;
      for (const auto& segment : segments) {
        if (Overlap(start, end, segment.start, segment.end) <= 1e-9) continue;
        if (covering != nullptr || segment.start > start + 1e-9 ||
            segment.end + 1e-9 < end) {
    ...
    const auto* activity = unique_covering_segment(
        pipeline.speakers_, containing_interval->start,
        containing_interval->end);
    const auto* phrase_primary = unique_covering_segment(
        pipeline.primary_speakers_, item.start, item.end);
    const auto* tail_primary = unique_covering_segment(
        pipeline.primary_speakers_, tail_unit->start, tail_unit->end);
    if (activity == nullptr || phrase_primary == nullptr ||
        tail_primary == nullptr || activity->speaker.empty() ||
        activity->speaker_id != current_identity ||
        phrase_primary->speaker != activity->speaker ||
        phrase_primary->speaker_id != current_identity ||
        tail_primary->speaker_id.empty() ||
        tail_primary->speaker_id == current_identity) {
      return std::nullopt;
    }
    const auto initial = initial_identities.find(activity->speaker);
    if (initial == initial_identities.end() || initial->second.second.empty() ||
        initial->second.second == current_identity ||
        initial->second.second == tail_primary->speaker_id) {
      return std::nullopt;
    }
    const std::string& candidate_identity = initial->second.second;
    ...
    const auto phrase_robust =
        ranked_pair(item.robust_scores, phrase_duration);
    if (!phrase_session || !phrase_robust ||
        phrase_session->first.speaker_id != candidate_identity ||
    ...
    const auto interval_session = ranked_pair(
        containing_interval->session_scores, interval_duration);
    const auto interval_robust = ranked_pair(
        containing_interval->robust_scores, interval_duration);
    if (!interval_session || !interval_robust ||
        interval_session->first.speaker_id != current_identity ||
        interval_robust->first.speaker_id != current_identity ||
        interval_session->second_id != candidate_identity ||
    ...
    for (const auto& candidate : pipeline.voiceprint_vad_) {
      if (candidate.kind != "vad" ||
          candidate.start > containing_interval->start + tolerance + 1e-9 ||
          candidate.end + tolerance + 1e-9 < containing_interval->end) {
        continue;
      }
      if (containing_vad != nullptr) return std::nullopt;
      containing_vad = &candidate;
    }
    if (containing_vad == nullptr || !containing_vad->embedding_available ||
        !containing_vad->robust_gallery_complete) {
      return std::nullopt;
    }
    const double vad_duration = containing_vad->end - containing_vad->start;
    if (vad_duration + 1e-9 < pipeline.config_.voiceprint_short_max_sec) {
      return std::nullopt;
    }
    ...
    const auto complete_session =
        ranked_pair(complete_source->session_scores, complete_duration);
    const auto complete_robust =
        ranked_pair(complete_source->robust_scores, complete_duration);
    auto expected_complete_pair = [&](const auto& view) {
      return (view.first.speaker_id == candidate_identity &&
              view.second_id == current_identity) ||
             (view.first.speaker_id == current_identity &&
              view.second_id == candidate_identity);
    };
    if (!complete_session || !complete_robust ||
        !expected_complete_pair(*complete_session) ||
        !expected_complete_pair(*complete_robust) ||
        (complete_session->first.speaker_id != candidate_identity &&
         complete_robust->first.speaker_id != candidate_identity) ||
        complete_session->first.score_pass ||
        complete_session->first.margin_pass ||
        complete_robust->first.score_pass ||
        complete_robust->first.margin_pass) {
      return std::nullopt;
    }
    return candidate_identity;
  };
  ...
  struct TwoUnitPrimaryHandoffSelection {
    int source_start = 0;
    int source_end = 0;
    std::string speaker_id;
  };
  auto two_unit_primary_handoff_challenge =
      [&](const SpeakerVoiceprintEvidence& item)
      -> std::optional<TwoUnitPrimaryHandoffSelection> {
    if (item.kind != "aligned_unit" || item.embedding_available ||
        item.text_id != text.id || item.end <= item.start ||
        pipeline.config_.voiceprint_punctuation.empty() ||
    ...
    struct UnitClause {
      const TextSeg* text = nullptr;
      int character_count = 0;
      int source_start = 0;
      int source_end = 0;
    };
    auto clause_for = [&](const SpeakerVoiceprintEvidence& unit)
        -> std::optional<UnitClause> {
      const TextSeg* unit_text = nullptr;
      for (const auto& candidate : pipeline.texts_) {
        if (candidate.id != unit.text_id) continue;
        if (unit_text != nullptr) return std::nullopt;
        unit_text = &candidate;
      }
      if (unit_text == nullptr) return std::nullopt;

      const auto unit_offsets = Utf8Offsets(unit_text->text);
      const int unit_character_count =
          static_cast<int>(unit_offsets.size()) - 1;
      if (unit.source_start < 0 ||
          unit.source_end > unit_character_count ||
          unit.source_end != unit.source_start + 1) {
        return std::nullopt;
      }
      const auto codepoint = unit_text->text.substr(
          unit_offsets[unit.source_start],
          unit_offsets[unit.source_end] - unit_offsets[unit.source_start]);
      if (codepoint == " " || codepoint == "\t" || codepoint == "\n" ||
          codepoint == "\r" || punctuation.count(codepoint) != 0) {
        return std::nullopt;
      }

      int source_end = unit.source_end;
      while (source_end < unit_character_count) {
        const auto trailing = unit_text->text.substr(
            unit_offsets[source_end],
            unit_offsets[source_end + 1] - unit_offsets[source_end]);
        if (punctuation.count(trailing) == 0) break;
        ++source_end;
      }
      if (source_end == unit.source_end) return std::nullopt;
      return UnitClause{unit_text, unit_character_count, unit.source_start,
                        source_end};
    };
    ...
    if (containing_vad == nullptr ||
        containing_vad->end <= containing_vad->start) {
      return std::nullopt;
    }
    ...
    for (const auto& candidate : pipeline.voiceprint_aligned_units_) {
      ...
      if (Overlap(containing_vad->start, containing_vad->end,
                  candidate.start, candidate.end) <= 1e-9) {
        continue;
      }
      if (candidate.start + 1e-9 < containing_vad->start ||
          candidate.end > containing_vad->end + 1e-9) {
        return std::nullopt;
      }
      units.push_back(&candidate);
    }
    if (units.size() != 2) return std::nullopt;
    ...
    if (&item != units[0] && &item != units[1]) return std::nullopt;

    const auto left_clause = clause_for(*units[0]);
    const auto right_clause = clause_for(*units[1]);
    if (!left_clause || !right_clause) return std::nullopt;
    if (units[0]->text_id == units[1]->text_id) {
      if (left_clause->source_end != units[1]->source_start) {
        return std::nullopt;
      }
    } else {
      if (left_clause->source_end != left_clause->character_count ||
          units[1]->source_start != 0 ||
          !NearEqual(left_clause->text->end, right_clause->text->start)) {
        return std::nullopt;
      }
    }

    const double unit_gap = units[1]->start - units[0]->end;
    if (unit_gap + 1e-9 < pipeline.config_.align_snap_pause_sec ||
        unit_gap - tolerance + 1e-9 >=
            pipeline.config_.voiceprint_primary_consensus_min_sec) {
      return std::nullopt;
    }

    auto covering_primary = [&](const SpeakerVoiceprintEvidence& unit)
        -> const SpeakerSeg* {
      const SpeakerSeg* covering = nullptr;
      int overlap_count = 0;
      for (const auto& primary : pipeline.primary_speakers_) {
        if (Overlap(unit.start, unit.end, primary.start, primary.end) <=
            1e-9) {
          continue;
        }
        ++overlap_count;
        if (primary.start <= unit.start + tolerance &&
            primary.end + tolerance + 1e-9 >= unit.end) {
          covering = &primary;
        }
      }
      return overlap_count == 1 ? covering : nullptr;
    };
    const SpeakerSeg* left_primary = covering_primary(*units[0]);
    const SpeakerSeg* right_primary = covering_primary(*units[1]);
    if (left_primary == nullptr || right_primary == nullptr ||
        left_primary == right_primary || left_primary->speaker_id.empty() ||
        right_primary->speaker_id.empty() ||
        left_primary->speaker_id == right_primary->speaker_id ||
        left_primary->end > right_primary->start + 1e-9) {
      return std::nullopt;
    }
    const std::string& activity_identity = left_primary->speaker_id;
    const std::string& vad_identity = right_primary->speaker_id;

    const SpeakerSeg* covering_activity = nullptr;
    for (const auto& activity : pipeline.speakers_) {
      if (Overlap(units[0]->start, units[0]->end, activity.start,
                  activity.end) <= 1e-9 &&
          Overlap(units[1]->start, units[1]->end, activity.start,
                  activity.end) <= 1e-9) {
        continue;
      }
      if (activity.start <= units[0]->start + tolerance &&
          activity.end + tolerance + 1e-9 >= units[1]->end &&
          activity.speaker_id == activity_identity) {
        if (covering_activity != nullptr) return std::nullopt;
        covering_activity = &activity;
        continue;
      }
      return std::nullopt;
    }
    ...
    const auto vad_robust =
        ranked_pair(containing_vad->robust_scores, vad_duration);
    auto expected_vad_pair = [&](const auto& view) {
      return view.first.speaker_id == vad_identity &&
             view.second_id == activity_identity && view.first.score_pass &&
             view.first.margin_pass;
    };
    if (!vad_session || !vad_robust || !expected_vad_pair(*vad_session) ||
        !expected_vad_pair(*vad_robust)) {
      return std::nullopt;
    }

    const bool is_left = &item == units[0];
    const auto& clause = is_left ? *left_clause : *right_clause;
    const std::string& candidate_identity =
        is_left ? activity_identity : vad_identity;
    std::string current_identity;
    for (int index = clause.source_start; index < clause.source_end; ++index) {
      if (!labels[index].voiceprint || labels[index].speaker_id.empty() ||
          labels[index].reason.rfind("voiceprint_direct_", 0) != 0) {
        return std::nullopt;
      }
      if (current_identity.empty()) {
    ...
    if ((current_identity != activity_identity &&
         current_identity != vad_identity) ||
        current_identity == candidate_identity) {
      return std::nullopt;
    }
    return TwoUnitPrimaryHandoffSelection{
        clause.source_start, clause.source_end, candidate_identity};
  };
  ...
  struct CrossVadPhraseTailSelection {
    int source_start = 0;
    int source_end = 0;
    std::string speaker_id;
  };
  auto cross_vad_phrase_tail_reconstruction_challenge = [&](const auto& item)
      -> std::optional<CrossVadPhraseTailSelection> {
    const double phrase_duration = item.end - item.start;
    if (item.kind != "punctuation_phrase" || item.text_id != text.id ||
        item.source_start != 0 || item.source_end > character_count ||
        item.source_end <= item.source_start || !item.embedding_available ||
    ...
    const SpeakerVoiceprintEvidence* following = nullptr;
    for (const auto* candidate : source_evidence) {
      if (candidate->kind != "business_interval" ||
          candidate->text_id != item.text_id) {
        continue;
      }
      if (candidate->source_start >= item.source_start &&
          candidate->source_end <= item.source_end) {
        components.push_back(candidate);
      } else if (candidate->source_start == item.source_end) {
        if (following != nullptr) return std::nullopt;
        following = candidate;
      }
    }
    ...
    if (components.size() != 2 || following == nullptr) {
      return std::nullopt;
    }
    const auto& leading = *components[0];
    const auto& tail = *components[1];
    const double leading_duration = leading.end - leading.start;
    const double tail_duration = tail.end - tail.start;
    const double following_duration = following->end - following->start;
    if (leading.source_start != item.source_start ||
        leading.source_end != tail.source_start ||
        tail.source_end != item.source_end ||
        following->source_start != item.source_end ||
        !NearEqual(leading.start, item.start) ||
        !NearEqual(tail.end, item.end) || !leading.embedding_available ||
        !leading.robust_gallery_complete || tail.embedding_available ||
        ...
        following_duration + 1e-9 <
            pipeline.config_.voiceprint_primary_consensus_min_sec ||
        leading.end > tail.start + 1e-9 ||
        tail.end > following->start + 1e-9 ||
        tail.start - leading.end + 1e-9 <
            pipeline.config_.align_snap_pause_sec) {
      return std::nullopt;
    }
    ...
    int visible_index = -1;
    int visible_count = 0;
    for (int index = tail.source_start; index < tail.source_end; ++index) {
      const std::string codepoint = text.text.substr(
          offsets[index], offsets[index + 1] - offsets[index]);
      if (codepoint == " " || punctuation.count(codepoint) != 0) continue;
      visible_index = index;
      ++visible_count;
    }
    const std::string final_codepoint = text.text.substr(
        offsets[tail.source_end - 1],
        offsets[tail.source_end] - offsets[tail.source_end - 1]);
    if (visible_count != 1 || visible_index != tail.source_start ||
        punctuation.count(final_codepoint) == 0) {
      return std::nullopt;
    }
    ...
    for (const auto* candidate : source_evidence) {
      if (candidate->kind != "aligned_unit" ||
          candidate->text_id != item.text_id ||
          candidate->source_end <= tail.source_start ||
          candidate->source_start >= tail.source_end) {
        continue;
      }
      if (candidate->end <= candidate->start || tail_unit != nullptr ||
          candidate->source_start != visible_index ||
          candidate->source_end != visible_index + 1 ||
          !NearEqual(candidate->start, tail.start) ||
          !NearEqual(candidate->end, tail.end)) {
        return std::nullopt;
      }
      tail_unit = candidate;
    }
    if (tail_unit == nullptr) return std::nullopt;

    std::string leading_identity;
    std::string leading_native_identity;
    for (int index = leading.source_start; index < leading.source_end;
      ...
      if (!labels[index].voiceprint ||
          ...
          labels[index].base_reason != "sole_diar_support" ||
          labels[index].speaker_id.empty() ||
          labels[index].base_speaker_id.empty()) {
        return std::nullopt;
      }
      if (leading_identity.empty()) {
        leading_identity = labels[index].speaker_id;
        leading_native_identity = labels[index].base_speaker_id;
      } else if (labels[index].speaker_id != leading_identity ||
                 labels[index].base_speaker_id != leading_native_identity) {
        return std::nullopt;
      }
    }
    std::string following_identity;
    auto uniform_native = [&](int source_start, int source_end) {
      std::string identity;
      for (int index = source_start; index < source_end; ++index) {
        if (labels[index].voiceprint ||
            labels[index].reason != "sole_diar_support" ||
        ...
        if (identity.empty()) {
          identity = labels[index].speaker_id;
        } else if (labels[index].speaker_id != identity) {
          return std::string();
        }
      }
      return identity;
    };
    const std::string tail_identity =
        uniform_native(tail.source_start, tail.source_end);
    following_identity =
        uniform_native(following->source_start, following->source_end);
    if (leading_identity.empty() || leading_native_identity.empty() ||
        tail_identity.empty() || tail_identity != following_identity ||
        leading_identity == leading_native_identity ||
        leading_identity == tail_identity ||
        leading_native_identity == tail_identity) {
      return std::nullopt;
    }

    auto unique_covering_slot = [&](const std::vector<SpeakerSeg>& segments,
                                    double start, double end,
      ...
      for (const auto& segment : segments) {
        if (Overlap(start, end, segment.start, segment.end) <= 1e-9) continue;
        if (covering != nullptr || segment.speaker_id != identity ||
            segment.speaker.empty() || segment.start > start + 1e-9 ||
            segment.end + 1e-9 < end) {
          return std::nullopt;
        }
        covering = &segment;
    ...
    const auto leading_activity = unique_covering_slot(
        pipeline.speakers_, leading.start, leading.end,
        leading_native_identity);
    const auto leading_primary = unique_covering_slot(
        pipeline.primary_speakers_, leading.start, leading.end,
        leading_native_identity);
    const auto tail_activity = unique_covering_slot(
        pipeline.speakers_, tail.start, tail.end, tail_identity);
    const auto tail_primary = unique_covering_slot(
        pipeline.primary_speakers_, tail.start, tail.end, tail_identity);
    const auto following_activity = unique_covering_slot(
        pipeline.speakers_, following->start, following->end, tail_identity);
    const auto following_primary = unique_covering_slot(
        pipeline.primary_speakers_, following->start, following->end,
        tail_identity);
    if (!leading_activity || !leading_primary || !tail_activity ||
        !tail_primary || !following_activity || !following_primary ||
        *leading_activity != *leading_primary ||
        *tail_activity != *tail_primary ||
        *following_activity != *following_primary ||
        *tail_activity != *following_activity ||
        *leading_activity == *tail_activity) {
      return std::nullopt;
    }

    const auto leading_session =
        ranked_pair(leading.session_scores, leading_duration);
    const auto leading_robust =
        ranked_pair(leading.robust_scores, leading_duration);
    if (!leading_session || !leading_robust ||
        leading_session->first.speaker_id != leading_identity ||
        leading_robust->first.speaker_id != leading_identity ||
        leading_session->second_id != leading_native_identity ||
        leading_robust->second_id != leading_native_identity ||
        !leading_session->first.score_pass ||
        !leading_session->first.margin_pass ||
        !leading_robust->first.score_pass ||
        !leading_robust->first.margin_pass) {
      return std::nullopt;
    }

    const auto phrase_session =
        ranked_pair(item.session_scores, phrase_duration);
    const auto phrase_robust = ranked_pair(item.robust_scores, phrase_duration);
    if (!phrase_session || !phrase_robust ||
        phrase_session->first.speaker_id != leading_native_identity ||
        phrase_session->second_id != leading_identity ||
        phrase_robust->first.speaker_id != leading_identity ||
        phrase_robust->second_id != leading_native_identity ||
        phrase_session->first.score_pass || phrase_robust->first.score_pass ||
        phrase_session->first.margin_pass ==
            phrase_robust->first.margin_pass) {
      return std::nullopt;
    }

    const auto following_session =
        ranked_pair(following->session_scores, following_duration);
    const auto following_robust =
        ranked_pair(following->robust_scores, following_duration);
    if (!following_session || !following_robust ||
        following_session->first.speaker_id != tail_identity ||
        following_robust->first.speaker_id != tail_identity) {
      return std::nullopt;
    }

    auto unique_containing_vad = [&](double start, double end)
        -> const SpeakerVoiceprintEvidence* {
      const SpeakerVoiceprintEvidence* containing = nullptr;
      for (const auto& candidate : pipeline.voiceprint_vad_) {
        if (candidate.kind != "vad" || candidate.start > start + 1e-9 ||
            candidate.end + 1e-9 < end) {
          continue;
        }
        if (containing != nullptr || !candidate.embedding_available ||
            !candidate.robust_gallery_complete) {
          return nullptr;
        }
    ...
    const auto* leading_vad =
        unique_containing_vad(leading.start, leading.end);
    const auto* tail_vad = unique_containing_vad(tail.start, tail.end);
    if (leading_vad == nullptr || tail_vad == nullptr ||
        leading_vad == tail_vad ||
        tail_vad->start > tail.start + 1e-9 ||
        tail.start - tail_vad->start >
            pipeline.config_.align_boundary_split_tolerance_sec + 1e-9 ||
        tail_vad->end <= following->start + 1e-9 ||
        tail_vad->start - leading_vad->end + 1e-9 <
            pipeline.config_.align_snap_pause_sec) {
      return std::nullopt;
    }

    const double leading_vad_duration = leading_vad->end - leading_vad->start;
    const auto leading_vad_session =
        ranked_pair(leading_vad->session_scores, leading_vad_duration);
    const auto leading_vad_robust =
        ranked_pair(leading_vad->robust_scores, leading_vad_duration);
    if (!leading_vad_session || !leading_vad_robust ||
        leading_vad_session->first.speaker_id != leading_identity ||
        leading_vad_robust->first.speaker_id != leading_identity ||
        leading_vad_session->second_id != leading_native_identity ||
        leading_vad_robust->second_id != leading_native_identity ||
        !leading_vad_session->first.score_pass ||
        !leading_vad_robust->first.score_pass ||
        leading_vad_session->first.margin_pass ==
            leading_vad_robust->first.margin_pass) {
      return std::nullopt;
    }

    const double tail_vad_duration = tail_vad->end - tail_vad->start;
    const auto tail_vad_session =
        ranked_pair(tail_vad->session_scores, tail_vad_duration);
    const auto tail_vad_robust =
        ranked_pair(tail_vad->robust_scores, tail_vad_duration);
    if (!tail_vad_session || !tail_vad_robust ||
        tail_vad_session->first.speaker_id != tail_identity ||
        tail_vad_robust->first.speaker_id != tail_identity ||
        tail_vad_session->second_id != leading_identity ||
        tail_vad_robust->second_id != leading_identity ||
        !tail_vad_session->first.score_pass ||
        !tail_vad_session->first.margin_pass ||
        !tail_vad_robust->first.score_pass ||
        !tail_vad_robust->first.margin_pass) {
      return std::nullopt;
    }

    return CrossVadPhraseTailSelection{tail.source_start, tail.source_end,
                                       leading_identity};
  };
  auto complete_source_aligned_vad_challenge = [&](const auto& item)
    ...
    bool malformed_partition_evidence = false;
    for (const auto& candidate : text_voiceprint) {
      if ((candidate.kind != "aligned_unit" &&
           candidate.kind != "business_interval") ||
          candidate.end <= candidate.start) {
      ...
      if (candidate.text_id != text.id || candidate.source_start < 0 ||
          candidate.source_end > character_count ||
          candidate.source_end <= candidate.source_start) {
        malformed_partition_evidence = true;
      }
    }
    ...
    const SpeakerVoiceprintEvidence* unanchored_interval = nullptr;
    for (const auto* candidate : source_evidence) {
      if (candidate->kind != "business_interval" ||
          candidate->end <= candidate->start) {
        continue;
      ...
      if (!anchored) {
        if (unanchored_interval != nullptr) return std::nullopt;
        unanchored_interval = candidate;
      }
      for (int index = candidate->source_start; index < candidate->source_end;
           ++index) {
    ...
    if (unanchored_interval != nullptr) {
      if (malformed_partition_evidence ||
          pipeline.config_.align_snap_pause_sec <= 0.0 ||
          pipeline.config_.voiceprint_punctuation.empty() ||
          unanchored_interval->source_end - unanchored_interval->source_start !=
              1) {
        return std::nullopt;
      }
      const int missing_index = unanchored_interval->source_start;
      if (zero_duration_alignment_count[missing_index] != 1) {
        return std::nullopt;
      }
      const std::string missing_codepoint =
          text.text.substr(offsets[missing_index],
                           offsets[missing_index + 1] - offsets[missing_index]);
      std::set<std::string> punctuation;
      const auto punctuation_offsets =
          Utf8Offsets(pipeline.config_.voiceprint_punctuation);
      for (std::size_t index = 0; index + 1 < punctuation_offsets.size();
      ...
      const SpeakerVoiceprintEvidence* previous_unit = nullptr;
      const SpeakerVoiceprintEvidence* following_unit = nullptr;
      for (const auto* unit : aligned_units) {
        if (unit->source_end - unit->source_start != 1 ||
            (unit->source_start <= missing_index &&
             unit->source_end > missing_index)) {
          return std::nullopt;
        }
        if (unit->source_end == missing_index) {
          if (previous_unit != nullptr) return std::nullopt;
          previous_unit = unit;
        }
        if (unit->source_start == missing_index + 1) {
          if (following_unit != nullptr) return std::nullopt;
          following_unit = unit;
        }
      }
      if (previous_unit == nullptr || following_unit == nullptr ||
          previous_unit->end > following_unit->start + 1e-9 ||
          following_unit->start - previous_unit->end + 1e-9 >=
              pipeline.config_.align_snap_pause_sec ||
          unanchored_interval->start > previous_unit->end + 1e-9 ||
          unanchored_interval->end + 1e-9 < following_unit->start) {
        return std::nullopt;
      }
      alignment_dropout = true;
    }
    ...
    auto coverage_by_identity = [&](const std::vector<SpeakerSeg>& segments) {
      ...
      for (const auto& segment : segments) {
        const double overlap =
            Overlap(aligned_start, aligned_end, segment.start, segment.end);
      ...
      for (auto& [identity, spans] : intervals) {
        coverage[identity] = CoveredDuration(MergeIntervals(std::move(spans)));
      }
      return coverage;
    };
    ...
    const auto primary_coverage =
        coverage_by_identity(pipeline.primary_speakers_);
    ...
    for (const auto& primary : pipeline.primary_speakers_) {
      ...
      const double overlap =
          Overlap(aligned_start, aligned_end, primary.start, primary.end);
      if (overlap > 1e-9) {
    ...
    auto local_pair_tied = [&](const auto& scores,
                               const std::string& nonlocal_identity) {
      std::optional<float> candidate_score;
      std::optional<float> incumbent_score;
      std::set<std::string> identities;
      for (const auto& score : scores) {
        if (!identities.insert(score.speaker_id).second) return false;
        if (score.speaker_id == candidate_identity) {
          candidate_score = score.score;
        } else if (score.speaker_id == incumbent_identity) {
          incumbent_score = score.score;
        }
      }
      return candidate_score && incumbent_score &&
             nonlocal_identity != candidate_identity &&
             nonlocal_identity != incumbent_identity &&
             std::fabs(*candidate_score - *incumbent_score) + 1e-9f <
                 pipeline.config_.voiceprint_short_min_margin;
    };
    const bool phrase_contains_candidate =
        phrase_session_pair && phrase_robust_pair &&
        pair_contains(*phrase_session_pair) &&
        pair_contains(*phrase_robust_pair);
    const bool dropout_local_pair_tie =
        alignment_dropout && phrase_session && phrase_robust &&
        phrase_session->speaker_id == phrase_robust->speaker_id &&
        activity_coverage.count(phrase_session->speaker_id) == 0 &&
        primary_coverage.count(phrase_session->speaker_id) == 0 &&
        local_pair_tied(phrase->session_scores, phrase_session->speaker_id) &&
        local_pair_tied(phrase->robust_scores, phrase_robust->speaker_id);
    if (!phrase_session || !phrase_robust || !phrase_session_pair ||
        ...
        (!phrase_contains_candidate && !dropout_local_pair_tie)) {
      return std::nullopt;
    }
  ...
  auto primary_alignment_gap_echo_challenge =
      [&](const SpeakerVoiceprintEvidence& item) -> std::optional<std::string> {
    const double duration = item.end - item.start;
    if (item.kind != "primary_alignment_gap_echo" ||
        item.text_id != text.id || !item.embedding_available ||
        !item.session_gallery_complete || !item.robust_gallery_complete ||
        duration + 1e-9 <
            pipeline.config_.voiceprint_primary_consensus_min_sec ||
        duration + 1e-9 >= pipeline.config_.voiceprint_short_max_sec ||
        pipeline.config_.voiceprint_punctuation.empty()) {
    ...
    if (matching_evidence != 1) return std::nullopt;

    auto identity_set = [](const auto& scores)
        -> std::optional<std::set<std::string>> {
      std::set<std::string> identities;
      for (const auto& score : scores) {
        if (score.speaker_id.empty() || !std::isfinite(score.score) ||
            !identities.insert(score.speaker_id).second) {
          return std::nullopt;
        }
      }
      if (identities.size() < 2) return std::nullopt;
      return identities;
    };
    const auto session_ids = identity_set(item.session_scores);
    const auto robust_ids = identity_set(item.robust_scores);
    std::set<std::string> active_ids;
    for (const auto& segment : pipeline.speakers_) {
      if (!segment.speaker_id.empty()) active_ids.insert(segment.speaker_id);
    }
    if (!session_ids || !robust_ids || *session_ids != *robust_ids ||
        *session_ids != active_ids) {
      return std::nullopt;
    }
    const auto session = select(item.session_scores, duration, true);
    const auto robust = select(item.robust_scores, duration, true);
    if (!session || !robust || session->speaker_id != robust->speaker_id) {
      return std::nullopt;
    }
    const std::string& selected = session->speaker_id;
    ...
    auto is_visible = [&](const std::string& codepoint) {
      return punctuation.count(codepoint) == 0 && codepoint != " " &&
             codepoint != "\t" && codepoint != "\n" && codepoint != "\r";
    };
    std::vector<std::pair<int, int>> phrases;
    int phrase_start = 0;
    for (int index = 0; index < character_count; ++index) {
      if (punctuation.count(source[index]) == 0) continue;
      if (std::any_of(source.begin() + phrase_start,
                      source.begin() + index + 1, is_visible)) {
        phrases.push_back({phrase_start, index + 1});
      }
      phrase_start = index + 1;
    }
    if (phrase_start < character_count &&
        std::any_of(source.begin() + phrase_start, source.end(), is_visible)) {
      phrases.push_back({phrase_start, character_count});
    }
    int target_phrase_index = -1;
    for (int index = 0; index < static_cast<int>(phrases.size()); ++index) {
      if (phrases[index].first == item.source_start &&
          phrases[index].second == item.source_end) {
        if (target_phrase_index >= 0) return std::nullopt;
        target_phrase_index = index;
      }
    }
    if (target_phrase_index <= 0) return std::nullopt;
    const auto& preceding = phrases[target_phrase_index - 1];
    const auto& following = phrases[target_phrase_index];
    if (preceding.second != following.first) return std::nullopt;
    auto visible_range = [&](const std::pair<int, int>& range) {
      std::vector<std::string> output;
      for (int index = range.first; index < range.second; ++index) {
        if (is_visible(source[index])) output.push_back(source[index]);
      }
      return output;
    };
    const auto preceding_visible = visible_range(preceding);
    const auto following_visible = visible_range(following);
    if (following_visible.empty() ||
    ...
    bool following_aligned = false;
    for (int index = following.first; index < following.second; ++index) {
      following_aligned = following_aligned || character_times[index].available;
    }
    if (!following_aligned) return std::nullopt;
    int containing_gap_count = 0;
    int previous_positive = -1;
    for (int index = preceding.first; index < preceding.second; ++index) {
      ...
      if (previous_positive >= 0) {
        const auto& left = character_times[previous_positive];
        const auto& right = character_times[index];
        if (right.start > left.end + 1e-9 &&
            item.start + 1e-9 >= left.end &&
            item.end <= right.start + 1e-9) {
          ++containing_gap_count;
        }
      }
      previous_positive = index;
    }
    if (containing_gap_count != 1) return std::nullopt;

    std::vector<const SpeakerSeg*> primary;
    primary.reserve(pipeline.primary_speakers_.size());
    for (const auto& segment : pipeline.primary_speakers_) {
    ...
    struct PrimaryBracket {
      const SpeakerSeg* preceding = nullptr;
      const SpeakerSeg* middle = nullptr;
      const SpeakerSeg* following = nullptr;
    };
    std::vector<PrimaryBracket> brackets;
    const double boundary_tolerance =
        pipeline.config_.align_boundary_split_tolerance_sec;
    for (std::size_t index = 1; index + 1 < primary.size(); ++index) {
      const auto* left = primary[index - 1];
      const auto* middle = primary[index];
      const auto* right = primary[index + 1];
      if (!NearEqual(middle->start, item.start) ||
          !NearEqual(middle->end, item.end) ||
          std::abs(left->end - middle->start) >
              boundary_tolerance + 1e-9 ||
          std::abs(middle->end - right->start) >
              boundary_tolerance + 1e-9 ||
          left->speaker_id.empty() || middle->speaker_id.empty() ||
          left->speaker_id != right->speaker_id ||
          left->speaker_id == middle->speaker_id ||
          middle->speaker_id != selected ||
          left->end - left->start + 1e-9 <
              pipeline.config_.voiceprint_primary_consensus_min_sec ||
          right->end - right->start + 1e-9 <
              pipeline.config_.voiceprint_primary_consensus_min_sec) {
        continue;
      }
      brackets.push_back({left, middle, right});
    }
    if (brackets.size() != 1) return std::nullopt;
    const auto& bracket = brackets.front();
    for (const auto* segment : primary) {
      if (segment == bracket.middle) continue;
      if (Overlap(item.start, item.end, segment->start, segment->end) > 1e-9) {
        return std::nullopt;
      }
    }
    auto has_full_matching_activity = [&](const SpeakerSeg& primary_segment) {
      int matching = 0;
      for (const auto& activity : pipeline.speakers_) {
        if (activity.speaker_id == primary_segment.speaker_id &&
            activity.speaker == primary_segment.speaker &&
            activity.start <= primary_segment.start + 1e-9 &&
            activity.end + 1e-9 >= primary_segment.end) {
          ++matching;
        }
      }
      return matching == 1;
    };
    if (!has_full_matching_activity(*bracket.preceding) ||
        !has_full_matching_activity(*bracket.middle) ||
        !has_full_matching_activity(*bracket.following)) {
      return std::nullopt;
    }

    const std::string& outer = bracket.preceding->speaker_id;
    ...
    for (int index = item.source_start; index < item.source_end; ++index) {
      if (labels[index].base_speaker_id != outer ||
          labels[index].speaker_id != outer ||
  ...
  struct SourceLeadingPrimaryPrefixSelection {
    int source_start = 0;
    int source_end = 0;
    std::string speaker_id;
  };
  auto source_leading_primary_prefix_challenge =
      [&](const SpeakerSeg& candidate)
      -> std::optional<SourceLeadingPrimaryPrefixSelection> {
    const double minimum =
        pipeline.config_.voiceprint_primary_consensus_min_sec;
    const double duration = candidate.end - candidate.start;
    if (!pipeline.config_.source_leading_primary_prefix_enabled ||
        candidate.speaker_id.empty() || candidate.end <= text.start + 1e-9 ||
        candidate.start + 1e-9 >= text.end || duration + 1e-9 < minimum ||
        duration + 1e-9 >= pipeline.config_.voiceprint_short_max_sec) {
      return std::nullopt;
    }
    ...
    const SpeakerSeg* following_primary = nullptr;
    for (const auto& primary : pipeline.primary_speakers_) {
      if (&primary == &candidate || !NearEqual(primary.start, candidate.end)) {
        continue;
      }
      if (following_primary != nullptr) return std::nullopt;
      following_primary = &primary;
    }
    if (following_primary == nullptr ||
        following_primary->speaker_id.empty() ||
        following_primary->speaker_id == candidate.speaker_id ||
        following_primary->end - following_primary->start + 1e-9 < minimum) {
      return std::nullopt;
    }

    auto unique_identity_set = [](const auto& scores)
        -> std::optional<std::set<std::string>> {
      std::set<std::string> identities;
      for (const auto& score : scores) {
        if (score.speaker_id.empty() ||
    ...
    const SpeakerVoiceprintEvidence* primary_evidence = nullptr;
    for (const auto& item : pipeline.voiceprint_) {
      if (item.kind != "primary_run" ||
          !NearEqual(item.start, candidate.start) ||
          !NearEqual(item.end, candidate.end)) {
        continue;
      }
      if (primary_evidence != nullptr) return std::nullopt;
      primary_evidence = &item;
    }
    if (primary_evidence == nullptr ||
        !primary_evidence->embedding_available ||
        !primary_evidence->session_gallery_complete ||
        !primary_evidence->robust_gallery_complete) {
      return std::nullopt;
    }
    const auto session_identities =
        unique_identity_set(primary_evidence->session_scores);
    const auto robust_identities =
        unique_identity_set(primary_evidence->robust_scores);
    const auto session = rank(primary_evidence->session_scores, duration);
    const auto robust = rank(primary_evidence->robust_scores, duration);
    if (!session_identities || !robust_identities ||
        *session_identities != *robust_identities || !session || !robust ||
        session->speaker_id != candidate.speaker_id ||
        robust->speaker_id != candidate.speaker_id || !session->score_pass ||
        !robust->score_pass ||
        (!session->margin_pass && !robust->margin_pass)) {
      return std::nullopt;
    }

    if (identity_coverage(pipeline.speakers_, candidate.start, candidate.end,
                          candidate.speaker_id) +
                1e-9 <
            duration ||
        identity_coverage(pipeline.speakers_, following_primary->start,
                          following_primary->end,
                          following_primary->speaker_id) +
                1e-9 <
            following_primary->end - following_primary->start) {
      return std::nullopt;
    }

    std::optional<SourceLeadingPrimaryPrefixSelection> selected;
    for (const auto* prefix : source_evidence) {
      if (prefix->kind != "business_interval" ||
          prefix->text_id != text.id || prefix->source_start < 0 ||
          prefix->source_end > character_count ||
          prefix->source_end <= prefix->source_start ||
          prefix->end <= prefix->start ||
          candidate.start > prefix->start + 1e-9 ||
          prefix->end > candidate.end + 1e-9) {
        continue;
      }
      ...
      for (int index = prefix->source_start; index < prefix->source_end;
           ++index) {
        if (!character_times[index].available ||
            labels[index].speaker_id != following_primary->speaker_id ||
            !labels[index].voiceprint) {
          prefix_valid = false;
          break;
        }
        if (first_aligned) {
          aligned_start = character_times[index].start;
          aligned_end = character_times[index].end;
          first_aligned = false;
        } else {
          aligned_start = std::min(aligned_start, character_times[index].start);
          aligned_end = std::max(aligned_end, character_times[index].end);
        }
      }
      bool candidate_source_already_present = false;
      for (int index = 0; index < prefix->source_start; ++index) {
        if (character_times[index].available &&
            labels[index].speaker_id == candidate.speaker_id &&
            Overlap(character_times[index].start, character_times[index].end,
                    candidate.start, candidate.end) > 1e-9) {
          candidate_source_already_present = true;
          break;
        }
      }
      if (!prefix_valid || first_aligned || candidate_source_already_present ||
          !NearEqual(prefix->start, aligned_start) ||
          !NearEqual(prefix->end, aligned_end) ||
          (prefix->source_start > 0 &&
           labels[prefix->source_start - 1].speaker_id ==
               following_primary->speaker_id)) {
        continue;
      }

      const SpeakerVoiceprintEvidence* following_interval = nullptr;
      for (const auto* item : source_evidence) {
        if (item->kind != "business_interval" || item->text_id != text.id ||
            item->source_start != prefix->source_end ||
            item->source_end <= item->source_start ||
            item->source_end > character_count || item->end <= item->start) {
          continue;
        }
        if (following_interval != nullptr) {
          following_interval = nullptr;
          break;
        }
        following_interval = item;
      }
      if (following_interval == nullptr) continue;

      bool following_labels_match = true;
      for (int index = following_interval->source_start;
           index < following_interval->source_end; ++index) {
        if (labels[index].speaker_id != following_primary->speaker_id) {
          following_labels_match = false;
          break;
        }
      }
      const double gap = following_interval->start - aligned_end;
      const double tolerance =
          pipeline.config_.align_boundary_split_tolerance_sec;
      if (!following_labels_match || gap <= 1e-9 ||
          gap > pipeline.config_.align_snap_pause_sec + 1e-9 ||
          candidate.end + 1e-9 < aligned_end ||
          candidate.end > following_interval->start + tolerance + 1e-9 ||
          following_interval->start + tolerance + 1e-9 <
              following_primary->start ||
          following_interval->end > following_primary->end + tolerance +
                                        1e-9) {
        continue;
      }

      bool conflicting_activity = false;
      for (const auto& activity : pipeline.speakers_) {
        if (Overlap(aligned_start, aligned_end, activity.start, activity.end) <=
            1e-9) {
          continue;
        }
        if (activity.speaker_id != candidate.speaker_id &&
            activity.speaker_id != following_primary->speaker_id) {
          conflicting_activity = true;
          break;
        }
      }
      if (conflicting_activity || selected) return std::nullopt;
      selected = SourceLeadingPrimaryPrefixSelection{
          prefix->source_start, prefix->source_end, candidate.speaker_id};
    }
    return selected;
  };

  struct RightBoundedShortPrimaryUnitSelection {
    int source_start = 0;
    int source_end = 0;
    std::string speaker_id;
  };
  auto right_bounded_short_primary_unit_challenge =
      [&](const SpeakerSeg& candidate)
      -> std::optional<RightBoundedShortPrimaryUnitSelection> {
    const double minimum =
        pipeline.config_.voiceprint_primary_consensus_min_sec;
    const double clock_tolerance = pipeline.time_base_.Duration(1);
    const double duration = candidate.end - candidate.start;
    if (!pipeline.config_.right_bounded_short_primary_unit_enabled ||
        candidate.speaker.empty() || candidate.speaker_id.empty() ||
        candidate.start + 1e-9 < text.start ||
        candidate.end > text.end + 1e-9 ||
        duration + clock_tolerance < minimum ||
        duration + 1e-9 >= pipeline.config_.voiceprint_short_max_sec) {
      return std::nullopt;
    }
    ...
    for (const auto& primary : pipeline.primary_speakers_) {
      if (&primary != &candidate &&
          Overlap(candidate.start, candidate.end, primary.start,
                  primary.end) > 1e-9) {
        return std::nullopt;
      }
    }
    ...
    if (following_primary == nullptr ||
        following_primary->speaker_id.empty() ||
        following_primary->speaker_id == candidate.speaker_id ||
        following_primary->end - following_primary->start + clock_tolerance <
            minimum) {
      return std::nullopt;
    }
    const std::string& incumbent = following_primary->speaker_id;
    const double following_duration =
        following_primary->end - following_primary->start;
    if (identity_coverage(pipeline.speakers_, following_primary->start,
                          following_primary->end, incumbent) +
            1e-9 <
        following_duration) {
      return std::nullopt;
    }
    ...
    for (const auto& item : pipeline.voiceprint_) {
      if (item.kind == "primary_run" &&
          NearEqual(item.start, candidate.start) &&
          NearEqual(item.end, candidate.end) && item.embedding_available) {
        return std::nullopt;
      }
    }

    std::vector<const AlignedSourceUnit*> contained_units;
    std::vector<const AlignedSourceUnit*> partial_units;
    for (const auto& unit : aligned_source_units) {
      const bool intersects =
          unit.zero_duration
              ? unit.start + 1e-9 >= candidate.start &&
                    unit.start <= candidate.end + 1e-9
              : Overlap(candidate.start, candidate.end, unit.start,
                        unit.end) > 1e-9;
      if (!intersects) continue;
      if (!unit.source_complete) return std::nullopt;
      const bool contained =
          unit.zero_duration
              ? unit.start > candidate.start + 1e-9 &&
                    unit.start + 1e-9 < candidate.end
              : unit.start + 1e-9 >= candidate.start &&
                    unit.end <= candidate.end + 1e-9;
      (contained ? contained_units : partial_units).push_back(&unit);
    }
    if (contained_units.size() != 1) return std::nullopt;
    const auto& selected_unit = *contained_units.front();
    if (selected_unit.source_end != selected_unit.source_start + 1 ||
        (selected_unit.zero_duration &&
         zero_duration_alignment_count[selected_unit.source_start] != 1) ||
        (!selected_unit.zero_duration &&
         selected_unit.end - selected_unit.start <=
             pipeline.config_.align_boundary_split_tolerance_sec + 1e-9)) {
      return std::nullopt;
    }
    ...
    auto source_codepoint = [&](int index) {
      return text.text.substr(offsets[index],
                              offsets[index + 1] - offsets[index]);
    };
    auto is_space = [](const std::string& codepoint) {
      return codepoint == " " || codepoint == "\t" || codepoint == "\n" ||
             codepoint == "\r";
    };
    const std::string selected_codepoint =
        source_codepoint(selected_unit.source_start);
    if (is_space(selected_codepoint) ||
        punctuation.count(selected_codepoint) != 0) {
      return std::nullopt;
    }

    int writable_end = selected_unit.source_end;
    while (writable_end < character_count &&
           punctuation.count(source_codepoint(writable_end)) != 0) {
      ++writable_end;
    }
    if (writable_end >= character_count ||
        is_space(source_codepoint(writable_end)) ||
        labels[writable_end].speaker_id != incumbent) {
      return std::nullopt;
    }
    for (int index = selected_unit.source_start; index < writable_end;
         ++index) {
      if (labels[index].speaker_id != incumbent || !labels[index].voiceprint ||
          labels[index].reason != "voiceprint_direct_regular") {
        return std::nullopt;
      }
    }
    ...
    for (int index = 0; index < character_count; ++index) {
      if (labels[index].speaker_id != candidate.speaker_id ||
          !character_times[index].available) {
        continue;
      }
      if (Overlap(character_times[index].start, character_times[index].end,
                  candidate.start, candidate.end) > 1e-9) {
        return std::nullopt;
      }
    }

    if (!partial_units.empty()) {
      if (!selected_unit.zero_duration || partial_units.size() != 1) {
        return std::nullopt;
      }
      const auto& successor = *partial_units.front();
      if (successor.zero_duration ||
          successor.source_start != writable_end ||
          successor.source_end != writable_end + 1 ||
          !NearEqual(successor.start, selected_unit.start) ||
          successor.end <= candidate.end + 1e-9) {
        return std::nullopt;
      }
    }

    return RightBoundedShortPrimaryUnitSelection{
        selected_unit.source_start, writable_end, candidate.speaker_id};
  };
  ...
  for (const auto* item : evidence) {
    ...
    const auto source_leading_tail =
        source_leading_phrase_tail_isolation_challenge(*item);
    if (source_leading_tail) {
      apply(*item, *source_leading_tail,
    ...
    if (item->kind == "aligned_unit") {
      ...
      const bool primary_activity_consensus =
          dual_agreement && primary_activity_phrase_consensus(
                                *item, session->speaker_id);
      const bool preserve_three_run_middle_slot =
          !primary_activity_consensus &&
          has_three_run_middle_slot_handoff(*item, *session);
      if (!primary_activity_consensus &&
          has_sustained_native_handoff(*item, session->speaker_id)) {
        continue;
      }
      if (preserve_three_run_middle_slot) continue;
  ...
  for (const auto* item : source_evidence) {
    const auto selected =
        cross_vad_phrase_tail_reconstruction_challenge(*item);
    if (!selected) continue;
    const std::string label = speaker_label(selected->speaker_id);
    for (int index = selected->source_start; index < selected->source_end;
         ++index) {
  ...
  for (const auto& item : pipeline.voiceprint_aligned_units_) {
    const auto selected = two_unit_primary_handoff_challenge(item);
    if (!selected) continue;
    const std::string label = speaker_label(selected->speaker_id);
    for (int index = selected->source_start; index < selected->source_end;
         ++index) {
  ...
  for (const auto* item : source_evidence) {
    const auto selected = primary_alignment_gap_echo_challenge(*item);
    if (!selected) continue;
    const std::string label = speaker_label(*selected);
    for (int index = item->source_start; index < item->source_end; ++index) {
      labels[index].speaker = label;
  ...
  for (const auto& primary : pipeline.primary_speakers_) {
    const auto selected =
        source_leading_primary_prefix_challenge(primary);
    if (!selected) continue;
    const std::string label = speaker_label(selected->speaker_id);
    for (int index = selected->source_start; index < selected->source_end;
         ++index) {
  ...
  std::optional<RightBoundedShortPrimaryUnitSelection>
      right_bounded_short_primary_unit;
  bool ambiguous_right_bounded_short_primary_unit = false;
  for (const auto& primary : pipeline.primary_speakers_) {
    const auto selected =
        right_bounded_short_primary_unit_challenge(primary);
    if (!selected) continue;
    if (right_bounded_short_primary_unit) {
      ambiguous_right_bounded_short_primary_unit = true;
      break;
    }
    right_bounded_short_primary_unit = selected;
  }
  if (right_bounded_short_primary_unit &&
      !ambiguous_right_bounded_short_primary_unit) {
    const auto& selected = *right_bounded_short_primary_unit;
    const std::string label = speaker_label(selected.speaker_id);
    for (int index = selected.source_start; index < selected.source_end;
         ++index) {
      labels[index].speaker = label;
  ...
  for (const auto& piece : projected_pieces) {
    ...
    if (label.reason == "primary_speaker_bracketed_aligned_unit_override") {
      ...
      entry.speaker_decision.speaker_source =
          ...
          "following_activity+speaker_identity_epoch+forced_alignment";
      entry.speaker_decision.reason = label.reason;
      ...
      if (label.reason ==
        ...
        entry.speaker_decision.speaker_source =
            ...
            "titanet_phrase+titanet_vad+robust_gallery+forced_alignment";
      } else if (
          label.reason ==
          "voiceprint_phrase_source_leading_tail_isolation_override") {
        entry.speaker_decision.speaker_source =
            "sortformer_activity+primary_top1+initial_slot+titanet_interval+"
          ...
          "voiceprint_aligned_unit_two_unit_primary_handoff_override") {
        entry.speaker_decision.speaker_source =
            "sortformer_activity+primary_top1+titanet_vad+robust_gallery+"
            "forced_alignment";
      } else if (
          label.reason ==
          "voiceprint_primary_alignment_gap_echo_override") {

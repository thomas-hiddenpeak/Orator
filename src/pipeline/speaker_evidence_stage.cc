...
SpeakerEvidenceStage::SpeakerEvidenceStage(SpeakerIdentityStage* identity,
  ...
  config_.phrase_min_sec = std::max(config_.min_embed_sec,
                                    config_.phrase_min_sec);
  config_.phrase_max_sec = std::max(config_.phrase_min_sec,
                                    config_.phrase_max_sec);
  config_.short_max_sec = std::max(config_.min_embed_sec,
                                   config_.short_max_sec);
  config_.minimum_gallery_size = std::max(2, config_.minimum_gallery_size);
}

std::vector<std::pair<int, int>>
SpeakerEvidenceStage::SplitPartialPhraseEdges(
...
SpeakerEvidenceStage::BuildVoiceprint(
  ...
  std::vector<Timeline::SpeakerVoiceprintEvidence> output;
  if (!config_.enabled) return output;
  const std::vector<std::string> active_ids = ActiveSpeakerIds(snapshot);
  ...
  for (auto& [text_id, entries] : entries_by_text) {
    ...
    for (const auto& entry : entries) {
      ...
      for (const auto& range : query_ranges) {
        ...
        const auto& evidence = evaluate(query_start, query_end);
        auto converted = ConvertEvidence(
            "business_interval:" + std::to_string(text_id) + ":" +
                std::to_string(ordinal++),
            "business_interval", text_id, range.first, range.second,
            query_start, query_end, evidence);
        business_intervals.push_back(converted);
  ...
  for (auto pair : BuildAdjacentBusinessPairs(
           business_intervals, config_.min_embed_sec,
           config_.short_max_sec)) {
    const auto& evidence = evaluate(pair.start, pair.end);
    output.push_back(ConvertEvidence(
        pair.evidence_id, pair.kind, pair.text_id, pair.source_start,
        pair.source_end, pair.start, pair.end, evidence));
  }
  ...
  for (const auto& text : snapshot.asr) {
    ...
    for (const auto& unit : alignment->second.units) {
      ...
      output.push_back(ConvertEvidence(
          "aligned_unit:" + std::to_string(text.id) + ":" +
              std::to_string(unit_ordinal++),
          "aligned_unit", text.id, source_start, source_end, unit.start,
          unit.end, evaluate(unit.start, unit.end)));
    }
  ...
  for (const auto& vad : snapshot.vad) {
    ...
    output.push_back(ConvertEvidence(
        "vad:" + std::to_string(vad_ordinal++), "vad", -1, 0, 0,
        vad.start, vad.end, evaluate(vad.start, vad.end)));
  }
  return output;
}

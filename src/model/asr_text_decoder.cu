...
void AsrTextDecoder::Embed(int token_id, float* out) const {
  const uint16_t* row = embed_.p + static_cast<size_t>(token_id) * config_.hidden_size;
...
void AsrTextDecoder::Forward(float* d_x, int Tq, int pos0) {
  ...
  *d_pos0 = pos0;
  ...
  for (int li = 0; li < config_.num_layers; ++li) {
    ...
    asr_ops::RmsNorm(d_x, L.in_ln.p, d_norm, Tq, Hh, eps);
    asr_gemm::F32ToBf16(d_norm, norm_bf16, static_cast<long>(Tq) * Hh);
    asr_gemm::LinearPre(norm_bf16, L.q_w.p, nullptr, d_q, Tq, Hh, Qd, 0);
    asr_gemm::LinearPre(norm_bf16, L.k_w.p, nullptr, d_k, Tq, Hh, KVd, 0);
    asr_gemm::LinearPre(norm_bf16, L.v_w.p, nullptr, d_v, Tq, Hh, KVd, 0);
    asr_ops::RmsNorm(d_q, L.q_norm.p, d_q, Tq * Hq, Dh, eps);
    asr_ops::RmsNorm(d_k, L.k_norm.p, d_k, Tq * Hkv, Dh, eps);
    asr_ops::RopeHalf(d_q, d_pos, Tq, Hq, Dh, config_.rope_theta);
    asr_ops::RopeHalf(d_k, d_pos, Tq, Hkv, Dh, config_.rope_theta);
    WriteKvCacheKernel<<<Blocks(static_cast<long>(Tq) * KVd), kThreads>>>(
        d_k, d_v, kc, vc, Tq, Hkv, Dh, d_pos0);
    CheckCudaError(cudaGetLastError(), __FILE__, __LINE__);
    const int warps = Tq * Hq;
    GqaCacheAttnKernel<<<Blocks(static_cast<long>(warps) * kWarp), kThreads>>>(
        d_q, kc, vc, d_attn, Tq, Hq, Hkv, Dh, d_pos0, scale);
    CheckCudaError(cudaGetLastError(), __FILE__, __LINE__);
    asr_gemm::Linear(d_attn, L.o_w.p, nullptr, d_proj, Tq, Qd, Hh, 0);
    AddResidualKernel<<<Blocks(static_cast<long>(Tq) * Hh), kThreads>>>(
        d_x, d_proj, static_cast<long>(Tq) * Hh);

    asr_ops::RmsNorm(d_x, L.post_ln.p, d_norm, Tq, Hh, eps);
    asr_gemm::F32ToBf16(d_norm, norm_bf16, static_cast<long>(Tq) * Hh);
    asr_gemm::LinearPre(norm_bf16, L.gate_w.p, nullptr, d_gate, Tq, Hh, I, 0);
    asr_gemm::LinearPre(norm_bf16, L.up_w.p, nullptr, d_up, Tq, Hh, I, 0);
    asr_ops::SwiGLU(d_gate, d_up, d_gate, Tq * I);
    asr_gemm::Linear(d_gate, L.down_w.p, nullptr, d_proj, Tq, I, Hh, 0);
    AddResidualKernel<<<Blocks(static_cast<long>(Tq) * Hh), kThreads>>>(
        d_x, d_proj, static_cast<long>(Tq) * Hh);
    CheckCudaError(cudaGetLastError(), __FILE__, __LINE__);
  }
  ...
  // Final norm on the LAST token only, then lm_head -> logits. All kernels run
  // on the default stream so they are already ordered; no mid-forward drain.
  if (prof) cudaEventRecord(e1);
  float* last = d_x + static_cast<size_t>(Tq - 1) * Hh;
  float* d_lastn = Work(9, Hh);
  asr_ops::RmsNorm(last, final_norm_.p, d_lastn, 1, Hh, eps);
...
void AsrTextDecoder::Prefill(const float* embeds, int T) {
...
void AsrTextDecoder::DecodeStep(const float* embed, int pos) {
  ...
  if (!step_x_) {
    ...
    step_pos_ = std::make_shared<UnifiedBuffer>(sizeof(int));
  ...
  *p = pos;
  ...
  // CUDA graphs are opt-in (ORATOR_ASR_GRAPH=1). On this 30W Orin the per-token
  // forward is compute-bound, not launch-bound, so graph capture did not help
  // and a non-default capture stream measured slower; default is eager on the
  // default stream (fastest observed path).
  const bool use_graph = std::getenv("ORATOR_ASR_GRAPH") != nullptr;
  if (!use_graph) {
    DecodeForwardOnStream(x, p, 0);
    CheckCudaError(cudaDeviceSynchronize(), __FILE__, __LINE__);
    return;
  }

  if (!graph_ready_) {
    DecodeForwardOnStream(x, p, capture_stream_);  // warm up (grow buffers)
    CheckCudaError(cudaStreamSynchronize(capture_stream_), __FILE__, __LINE__);
    cudaGraph_t graph;
    CheckCudaError(cudaStreamBeginCapture(capture_stream_, cudaStreamCaptureModeThreadLocal),
                   __FILE__, __LINE__);
    DecodeForwardOnStream(x, p, capture_stream_);
    CheckCudaError(cudaStreamEndCapture(capture_stream_, &graph), __FILE__, __LINE__);
    cudaGraphExec_t exec;
    CheckCudaError(cudaGraphInstantiate(&exec, graph, nullptr, nullptr, 0),
                   __FILE__, __LINE__);
    cudaGraphDestroy(graph);
    graph_exec_ = exec;
    graph_ready_ = true;
  }
  CheckCudaError(cudaGraphLaunch(static_cast<cudaGraphExec_t>(graph_exec_), capture_stream_),
                 __FILE__, __LINE__);
  CheckCudaError(cudaStreamSynchronize(capture_stream_), __FILE__, __LINE__);
}

int AsrTextDecoder::Argmax(int ban0, int ban1) {
  asr_ops::ArgmaxBanned(static_cast<float*>(d_logits_->data()), config_.vocab_size,
                        ban0, ban1, static_cast<int*>(d_argmax_->data()));
  CheckCudaError(cudaDeviceSynchronize(), __FILE__, __LINE__);
...
int AsrTextDecoder::ReadTokenId(const int* d_token) const {
  CheckCudaError(cudaDeviceSynchronize(), __FILE__, __LINE__);
  return *d_token;
}
...
void AsrTextDecoder::DecodeStepDevice(const int* d_token, int pos) {
  ...
  *p = pos;
  cache_len_ = pos + 1;
  EmbedGatherKernel<<<Blocks(Hh), kThreads>>>(embed_.p, d_token, x, Hh);
  CheckCudaError(cudaGetLastError(), __FILE__, __LINE__);
  DecodeForwardOnStream(x, p, 0);
}
...
std::vector<int> AsrTextDecoder::DecodeGreedy(int start_pos, int max_new,
  ...
  if (!d_tok_) {
    d_tok_ = std::make_shared<UnifiedBuffer>(sizeof(int));
    d_count_ = std::make_shared<UnifiedBuffer>(sizeof(int));
  }
  if (!d_out_ || d_out_->size() < sizeof(int) * static_cast<size_t>(max_new)) {
    d_out_ = std::make_shared<UnifiedBuffer>(sizeof(int) * static_cast<size_t>(max_new));
    if (graph_exec_) {
  ...
  // Prefill ran on the default stream and was NOT drained; sync before touching
  // managed scalars from the host (Tegra faults on host access to managed memory
  ...
  *cnt = 0;
  ...
  asr_ops::ArgmaxBanned(static_cast<float*>(d_logits_->data()), config_.vocab_size,
                        seed_ban ? eos0 : -1, seed_ban ? eos1 : -1, tok, 0);
  CheckCudaError(cudaDeviceSynchronize(), __FILE__, __LINE__);
  auto t_seed1 = clk();
  ...
  if (use_graph && !graph_ready_) {
    ...
    DecodeBodyImpl(0, ban0 ? eos0 : -1, ban0 ? eos1 : -1);  // warmup + token 0
    CheckCudaError(cudaDeviceSynchronize(), __FILE__, __LINE__);
    ++emitted;
  ...
  if (!have_graph) {
    ...
    while (emitted < max_new) {
      ...
      for (int b = 0; b < this_batch; ++b)
        DecodeBodyImpl(0, next_ban ? eos0 : -1, next_ban ? eos1 : -1);
      CheckCudaError(cudaDeviceSynchronize(), __FILE__, __LINE__);
      emitted += this_batch;
  ...
  while (emitted < max_new) {
    ...
    auto s0 = clk();
    for (int b = 0; b < this_batch; ++b)
      CheckCudaError(cudaGraphLaunch(g, capture_stream_), __FILE__, __LINE__);
    CheckCudaError(cudaStreamSynchronize(capture_stream_), __FILE__, __LINE__);

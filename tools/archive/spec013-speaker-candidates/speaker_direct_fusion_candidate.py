#!/usr/bin/env python3
"""Fuse current-interval TitaNet evidence into a frozen speaker view.

This reference-free generator permits only evidence measured on the current
business interval or its overlapping diar intervals. It never propagates a
speaker identity from a neighbouring turn or a local Sortformer slot.
"""
...
def aggregate_segment_scores(turn, active_ids):
    ...
    for segment in turn["diar_segments"]:
        ...
        if evidence.get("status") != "ok" or not allowed.issubset(scores):
            continue
...
def source_record(name, ranked, evidence_ids, config):
    speaker = config["speaker"]
    strong = score_gate(
        ranked,
    ...
    strength = "strong" if strong else "candidate" if candidate else None
    return {
        "name": name,
        "strength": strength,
        "ranked": ranked,
...
def select_identity(turn, active_ids, config):
    turn_ranked = (
    ...
    sources = [
        source_record("turn", turn_ranked, [str(turn["turn_id"])], config),
        source_record("diar_segment", segment_ranked, segment_ids, config),
    ]
    eligible = [source for source in sources if source["strength"]]
    if not eligible:
        return None, "no_eligible_direct_voiceprint", sources
    identities = {source["ranked"][0][0] for source in eligible}
    if len(identities) != 1:
        return None, "direct_voiceprint_conflict", sources
    selected = next(iter(identities))
    strength = (
        "strong" if any(source["strength"] == "strong" for source in eligible)
        else "candidate")
    source_name = (
        "combined" if len(eligible) == 2 else eligible[0]["name"])
    return selected, f"{strength}_{source_name}_voiceprint", sources


def decide_turn(turn, active_ids, config):
    baseline = turn["baseline_output"]
    baseline_id = baseline.get("speaker_id")
    selected, evidence_reason, sources = select_identity(
        turn, active_ids, config)
    changed = selected is not None and selected != baseline_id
    if selected is None:
        selected = baseline_id
        reason = f"baseline_{evidence_reason}"
    elif not changed:
        reason = f"baseline_confirmed_{evidence_reason}"
    else:
...
def build_candidate(evidence):
    ...
    return {
        ...
        "candidate_kind": "v21_direct_current_interval_voiceprint",
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(f"speaker direct fusion candidate: {error}")

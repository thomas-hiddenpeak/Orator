...
提交 `6dbc600e4eb5` 的双轮全长证据均从当前源码构建，并经真实增量
WebSocket 以 1.0x 输入速率运行 `3615.120` 秒。空注册表 Run A 的完整上下文
人工复核结果为 `514/556`（约 `92.45%`），重启后加载冻结注册表的 Run B 为
`513/556`（约 `92.27%`）。两轮均通过 Spec 013 的完整自然业务轮次说话人
归属门槛。

这不是 Spec 013 整体收官结论。说话人时长、固定 600 秒分块、逐说话人召回、
关键轮次、置信错误和可听边界偏移尚未完成签署。统一客户端现已默认在最后
一帧后直接发送 `end`，并分别记录可选 `flush`、`end` 和最后一帧到终态的
等待时间；120 秒真实语音机械验证为 `0.807 s`，但全长 A/B 尚未复验，因此
`<= 30 s` 全长终态门槛仍开放。ASR 准确率、独立 holdout 和发布签署同样不在
上述结论内。完整审计见
[current-commit-full-review-2026-07-17.md](specs/013-industrial-closing-validation/current-commit-full-review-2026-07-17.md)。

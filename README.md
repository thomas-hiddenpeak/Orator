...
v2.1 说话人业务管线已通过 `test.mp3` 规范场景的双轮全长验收。两轮均从
当前源码构建并经真实增量 WebSocket 以 1.0x 输入速率运行 `3615.120` 秒：
空注册表 Run A 经完整上下文人工复核为 `515/556`（约 `92.63%`），加载
冻结注册表的 Run B 为 `513/556`（约 `92.27%`）。两轮均超过 Spec 013
的 90% 说话人业务门槛。该结论只覆盖综合时间线中的说话人归属，不代表
ASR 准确率或独立 holdout 场景已经收官。

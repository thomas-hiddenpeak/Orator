...
过渡实验提交 `6b1cb79fa4f5` 的双轮全长证据均从干净源码构建，并经真实
增量 WebSocket 以 1.0x 输入速率运行 `3615.120` 秒。空注册表 Run A 与重启后
加载冻结注册表的 Run B 都完成了全部 556 个参考语境的正向及逆向人工复核；
后续 T102 上下文复核纠正了 `ref-0160` 的参考标签冲突和
`ref-0182` 的时间边界误判，两轮最终均为 `519/556`（约 `93.35%`），
独立通过 Spec 013 的自然业务轮次说话人归属门槛。
FR16ABN 修复了两轮 `ref-0090` 的延迟短确认，且没有形成长程或尾段身份漂移。

这不是 Spec 013 整体收官结论。`test.txt` 已是人工听审参考，不需要再次转写或
重听 556 条。固定 600 秒轮次和逐说话人轮次召回均已通过；人工复核确认 31 条
明确错人（约 `5.58%`），其中 22 条具有关键业务含义，另有 1 条关键发言缺失，
关键轮次和明确错人门禁
因此失败。说话人时长、逐说话人时长和源时间偏移仍待签署。源时间只按
`test.txt` 的整秒记录精度解释，不生成亚秒参考真值。统一客户端在最后一帧后直接
发送 `end`；本次全长 A/B 的终态等待分别为 `25.849 s` 和 `25.585 s`，机械
...
轮次分块、逐说话人及关键错误复核见
[speaker-gate-breakdown-review-2026-07-18.md](specs/013-industrial-closing-validation/speaker-gate-breakdown-review-2026-07-18.md)。
遥测修复证据见
...
`test.txt`、已记录的 `orator.toml` 和真实增量 WebSocket 路径。

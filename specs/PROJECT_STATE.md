...
- **Last updated**: 2026-07-19 (FR41 frozen candidate retained; closure open)
- **Branch**: `master`
- **Constitution**: v1.7.0
- **Speaker-business closure**: **BEST FROZEN FULL AVERAGE ABOVE FLOOR;
  CONJUNCTIVE NATURAL-TURN GATES FAIL; FULL CANONICAL CLOSURE OPEN**.
...
| Revisable comprehensive timeline (Spec 004) | T111 remains the frozen comparison baseline; FR29/FR30/FR31 rejected; FR32 real-path and FR33-FR41 frozen repairs retained; closure open | `ComprehensiveTimeline` stores typed diarization, ASR, VAD, alignment, voiceprint, and business tracks and publishes immutable snapshots/typed updates. Its VAD snapshot includes observed, active, and confirmed-silence frontiers used by scheduling-invariant ASR gating. `BusinessSpeakerPipeline` consumes typed `SpeakerEvidenceStage` output and owns orchestration/publication; the internal `SpeakerFusionPolicy` owns rule execution. T135's uniform complete-context reconciliation records T111 at `514/556`, FR29/T123 at `505/556`, and FR30/T133 at `497/556`. FR31's broad primary-return guard is rejected. FR32 repairs `ref-0154`; T142's independent full paths each record `506/556` after complete forward/reverse contextual review. FR33-FR41 then add nine separately diagnosed, deterministic frozen repairs; the current manually reconciled frozen candidate is `515/556`, all complete 600-second blocks and all four per-speaker natural-turn floors pass, and no new real-path result is claimed. Critical attribution and confident-wrong attribution still fail. FR16ABO's historical `518/556` predates uniform reconciliation and is not used for ranking. Checked-in VAD threshold is `0.5`. Speaker-time, ASR, and independent holdout remain open. |
...
- [specs/013-industrial-closing-validation/spec.md](013-industrial-closing-validation/spec.md) - **T135 reconciled the full T111/T123/T133 natural-turn ledgers; FR16ABO, both FR28 600-second promotions, FR29 full T123, FR30 full T133/T134, and FR31 remain rejected; FR32 completes full real-WebSocket promotion and FR33-FR41 complete bounded frozen reviews, but speaker-business closure remains open; T102/T084 remain open**. Complete forward/reverse contextual review records T111 at `514/556`, T123 at `505/556`, and T133 at `497/556` before FR32. T142's independent full FR32 paths each record `506/556`. Nine separately reviewed FR33-FR41 frozen repairs produce the current manually reconciled candidate of `515/556`, with no new real-path claim. All complete 600-second blocks and all four per-speaker natural-turn floors pass, but critical attribution and confident-wrong attribution still fail. Commit `d610de36ed13` closes T112 mechanically without changing speaker behavior. Checked-in `vad.threshold` is `0.5`. Speaker-time, per-speaker time, source-time-offset, ASR, browser/microphone, holdout, report-review, and release-tag gates remain open.

## 7. Immediate next step

Keep the streaming v2.1 `340/1/188/188` profile and the corrected T111
`514/556` frozen comparison fixed. FR33-FR41 produce a separately reviewed
current frozen candidate of `515/556`; it is not an accepted closing result.
...
per-speaker natural-turn floors now pass. None of these results is a new
real-WebSocket run; critical and confident-wrong attribution, speaker-time
sign-off, repeatability, and holdout gates remain open.

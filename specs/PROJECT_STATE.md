...
| Native Qwen3-ASR engine | One-second mismatch demonstrated; isolated 800-frame trained-window candidate active; contextual review pending | Stored stage fixtures report mel 3.9e-3, encoder 1.3e-3, and decoder argmax parity for their frozen fixtures. Spec 014 T061 pins official source `7c6daf7` and repairs stale oracle provenance. T062 demonstrates that all tested 100-frame standalone appends differ from their full-window slices, while complete 800-frame attention windows match exactly. Candidate `asr-final-full-context-decode` passes engineering but fails complete focused contextual review due to new critical assertions; its TOML switch is false and implementation is inactive evidence. Phase 3D adds typed `[asr].stream_window_mel_frames`, strictly limited to evidenced values 100/800 and included in resolved provenance. The 100-frame control passes engineering, and the checked-in TOML now activates only the 800-frame trained-window candidate for the exact focused capture. No 800-frame product output has yet been reviewed or accepted. A current full real-WebSocket baseline remains below ASR closing gates. Active-speech microphone, endpoint, candidate duration ladder, full A/B, and holdout acceptance remain open. Pure bf16 compute. |
...
| Incremental KV-cache ASR streaming (Spec 003) | Implemented; isolated 800-frame candidate active | Persistent KV cache, prefix rollback, and partial publication are implemented. The retained T011 numerical probe proves standalone locality for complete 800-mel-frame attention windows. T062 demonstrates that independently freezing 100-frame appends is not equivalent: all 16 tested slices differ and the maximum absolute tensor difference is `0.1759`, versus exact equality for both complete control windows. Source defaults remain `max_new_tokens=32`, `unfixed_chunks=2`, and `unfixed_tokens=5`; the stale Spec 003 statement that runtime uses 15 tokens is not code-authoritative. The full-context Final correction is rejected and disabled. Spec 014 Phase 3D exposes a TOML-owned append window, strictly limits it to the evidenced 100/800 values, and passes the 100-frame engineering control plus repeated exact 800-frame numerical gate. Checked-in TOML now uses 800 for the isolated focused candidate, so its Live text updates only when a complete trained window or Final tail is available; no UI acceptance follows. Historical script-derived CER and parameter-sweep claims are not accepted product evidence under Constitution 1.7.0. |
...
| Test suite | 75 configured CTest entries | The active Sortformer gates are bound to v2.1. Focused C++ tests cover typed speaker evidence, capture-ordered speaker-identity replay input, ASR full-context Final PCM/replacement/retraction, strict 100/800-frame streaming-window configuration, embedding-cache lifecycle, bounded precompute, final draining, accepted production fusion/abstention policy, and monotonic telemetry deadlines; active Python tests cover ASR oracle provenance, source/time/config invariants, immutable raw tracks, WebSocket manifests, telemetry, evidence integrity, replay inputs, and unjudged review packets. Thirty-three historical candidate tests are archived outside CTest. The checked-in 800-frame Phase 3D candidate passes all `75/75` entries in `50.77 s`; the final build emits no warning or error diagnostic. Browser and physical-microphone acceptance remain outside CTest. No automated result may assign correctness, aggregate accuracy, rank/select a candidate, or produce product acceptance. |
...
- [specs/014-asr-vad-closing/spec.md](014-asr-vad-closing/spec.md) - **Baseline and Live publication correction complete through 600 seconds; Chromium file flow and bounded room tone reviewed; two bounded Final candidates rejected; trained-window engineering control complete and isolated capture next; ASR semantics and voiced microphone remain open (2026-08-09)**. This focused spec freezes FR50 speaker behavior while establishing full contextual ASR, VAD endpoint, Live/Final, microphone, and browser evidence. The current full baseline has complete chronological and reverse contextual review of all 556 contributions. Commit `a1c8d1d` removes unchanged direct partial publication without changing final product tracks and passes warning-clean build, `74/74` tests, three directly reviewed silence sessions, repeated 120-second, 360-second, and 600-second real-WebSocket gates. Commits `b0eadbe` and `d09b13b` correct browser persistence and source-clock pacing; the clean 120-second Chromium flow passes complete mechanical and contextual review. A real physical-endpoint room-tone session at `d2ac5d1` has no substantive speech assertion, but the only board analog source cannot currently deliver controlled speech, so active microphone scenarios remain open. Complete direct rereading and a common-clock trace of `text_id=133` isolate one prompt-conditioned decoder factor: `语音识别` appears in Live and final ASR before alignment and matches the configured system prompt. Clean commit `5accc5f` tests and rejects an empty-system-prompt candidate. T061/T062 then pin the official implementation and demonstrate that independent 100-frame encoder appends violate the accepted 800-frame locality contract. Clean commit `5b6ba51` tests complete-segment full-context Final replacement in the same 102-second passage. Full chronological and reverse contextual review finds useful repairs but also new critical assertions about “雷总” and the signed agreement type, so T064 rejects and disables it before longer gates. Phase 3D now exposes `[asr].stream_window_mel_frames`, rejects values outside the evidenced 100/800 pair, passes the 100-frame engineering control and repeated exact 800-frame numerical gate, and leaves the isolated 800-frame real-WebSocket capture as the next product step. Every product conclusion is directly reviewed; ASR remains below closing requirements and the conditional FR50 speaker boundary remains unchanged. See [full-baseline-context-review-2026-08-09.md](014-asr-vad-closing/full-baseline-context-review-2026-08-09.md), [live-partial-publication-review-2026-08-09.md](014-asr-vad-closing/live-partial-publication-review-2026-08-09.md), [browser-persistence-review-2026-08-09.md](014-asr-vad-closing/browser-persistence-review-2026-08-09.md), [physical-microphone-review-2026-08-09.md](014-asr-vad-closing/physical-microphone-review-2026-08-09.md), [final-asr-prompt-causality-review-2026-08-09.md](014-asr-vad-closing/final-asr-prompt-causality-review-2026-08-09.md), and [streaming-encoder-boundary-review-2026-08-09.md](014-asr-vad-closing/streaming-encoder-boundary-review-2026-08-09.md). Raw artifacts remain under gitignored `artifacts/spec014/`.

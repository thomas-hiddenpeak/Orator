...
| GPU telemetry | Runtime/UI verified; long acceptance open | Compile-time default remains disabled; committed `orator.toml` enables 1 s samples. Runtime emits GPU utilization with source, CUDA unified-memory use, frequency, `VIN` system power, and rails; the client combines these with `tegrastats` CPU/RAM/temperature and requires both field and cadence coverage. The source-stable 120 s run passed at 115 runtime / 120 device samples. |
...
audible ledger, exact speaker-time totals, microphone/live-view acceptance, and
an independent holdout set. The bullets below are historical implementation and

...
- **Last updated**: 2026-08-10 (Phase 3I activates the exact official-greedy
  accumulated ASR candidate after all engineering gates; complete full-length
  capture and contextual review are next)
- **Branch**: `master`
- **Constitution**: v1.8.0
- **Active focused work**: **Spec 014 Phase 1 and Phase 2 are complete.** The
  ...
  prohibition, and accumulated-branch closure are superseded. Phase 3I now
  checks in the exact `accumulated_redecode`, 1000 ms, `ban_steps = 0`,
  trace-false candidate. Its exact configuration test passes, complete CTest
  passes `75/75` in `51.93` seconds, and a clean build emits no warning or
  error diagnostic. It now requires a 3615.120-second real-WebSocket run plus
  complete chronological and reverse contextual review of all 556 `test.txt`
  contributions before any product verdict. See
...
| Native Qwen3-ASR engine | Phase 3I full-length candidate active; product result pending | Stored stage fixtures report mel 3.9e-3, encoder 1.3e-3, and decoder argmax parity for their frozen fixtures. Spec 014 T061 pins official source `7c6daf7`. T062 demonstrates that tested 100-frame standalone appends differ from their full-window slices, while complete 800-frame attention windows match exactly. Phase 3G captures and completely reads 91 exact states, proving the accumulated transition faithful while identifying native first-three-token EOS suppression absent from official vLLM sampling. Phase 3H removes that suppression; its 102-second reading finds unresolved local defects but has no product-decision authority under Constitution 1.8.0. Phase 3I checks in the exact `accumulated_redecode`, 1000 ms, `ban_steps = 0`, trace-false candidate; exact configuration and all `75/75` tests pass. Complete `test.mp3` capture/review, active-speech microphone, full A/B, and holdout acceptance remain open. Pure bf16 compute. |
...
| Native ASR streaming (Specs 003 and 014) | Phase 3I candidate active; full semantic decision pending | Phase 3G's default-off trace captures all 91 one-second transitions; complete forward/reverse diagnostic review finds the state machine faithful and isolates native EOS suppression as one concrete official-contract mismatch. Phase 3I checks in `accumulated_redecode`, 1000 ms, `ban_steps = 0`, and trace false while every VAD, prompt, segment, model, alignment, speaker, time-base, and publication value stays fixed. Exact configuration, warning-clean build, and all `75/75` tests pass. Its 102-second review remains diagnostic and cannot reject the candidate, state an overall regression, or close the branch under Constitution 1.8.0. Complete canonical capture and dual contextual review are required before a product verdict. No second decoder parameter is authorized. Historical script-derived CER and parameter-sweep claims are not accepted product evidence. |

...
- **Last updated**: 2026-07-23 (FR50 completed current-config posterior
  provenance, deterministic 33-context A/B evidence capture, four independent
  full chronological/reverse contextual readings, and the manual T228 bounded
  topology decision; T229 implementation is next; no ledger or closure change)
- **Branch**: `master`
- **Constitution**: v1.7.0
- **Speaker-business closure**: **BEST FROZEN AND REAL-PATH NATURAL-TURN
  LEDGER `523/556`; CRITICAL GATES FAIL; FULL CANONICAL CLOSURE OPEN**. FR49's
...
| Test suite | 71 configured CTest entries | The active Sortformer gates are bound to v2.1. Focused C++ tests cover typed speaker evidence, capture-ordered speaker-identity replay input, embedding-cache lifecycle, bounded precompute, final draining, accepted production fusion/abstention policy, and monotonic telemetry deadlines; active Python tests cover source/time/config invariants, immutable raw tracks, WebSocket manifests, telemetry, evidence integrity, replay inputs, and unjudged review packets. Thirty-three historical candidate tests are archived outside CTest. FR49 adds a reference-free short-primary evidence exporter regression plus focused primary-run production and bounded fusion abstentions. The final clean build emits no warning or error diagnostic, and all `71/71` CTest entries pass in `53.22 s`. Browser and physical-microphone acceptance remain outside CTest. No automated result may assign correctness, aggregate accuracy, rank/select a candidate, or produce product acceptance. |
...
- [specs/013-industrial-closing-validation/spec.md](013-industrial-closing-validation/spec.md) - **FR50 current-config provenance, frozen all-residual evidence capture, four independent full chronological/reverse contextual readings, and manual T228 bounded-topology decision complete; T229 implementation active; FR49 remains the real-path candidate; speaker-business closure and T102/T084 remain open**. Earlier FR16ABO, FR28, FR29, FR30, and FR31 promotions remain rejected; FR32-FR47 are retained bounded historical repairs and FR46 is evidence-only. FR48's complete speaker-only reread and FR49's residual inventory establish the corrected pre-candidate real-path ledger at `521/556`. FR49's exact topology repairs `ref-0061` and `ref-0121`. Clean pushed commit `1f09052` then completes two 120-second runs, one 600-second run, and independent full empty/frozen-registry A/B captures. Every full artifact is read completely in chronological and reverse fixed-window context. The four readings manually retain `523/556`, with 27 confident-wrong, five missing, one uncertain, and the same 20 critical failures; no code evaluates correctness or produces the product verdict. FR50 has generated deterministic full A/B evidence trees for all 33 manually signed residuals and accepted controls after current-TOML v2.1 producer verification. T226's A/B chronological and T227's A/B reverse readings manually record and reconcile the earliest observed evidence-loss layer for every context without a forward/reverse disagreement. T228 manually authorizes one false-by-default, TOML-only experiment for the exact right-bounded single-codepoint short-primary topology shared by `ref-0327` and `ref-0417`, with explicit abstentions; it is not yet implemented, replayed, or accepted. Any ASR/VAD/alignment or speaker-evidence producer change invalidates the affected FR49 judgments and requires a fresh exact capture plus complete contextual review. No ledger or verdict changes. Checked-in `vad.threshold` is `0.5`. Speaker-time, per-speaker time, source-time-offset, ASR, browser/microphone, locked holdout, report-review, and release-tag gates remain open.

## 7. Immediate next step

Keep the streaming v2.1 `340/1/188/188` profile, checked-in TOML, exact FR49
A/B artifacts, registries, audio, reference, and clean implementation commit
`1f09052` fixed. FR50 is now the active speaker-only stage. It starts from the
33 manually signed residuals and accepted controls, and arranges independent A
and B evidence from Sortformer posterior/activity/primary, VAD, ASR, forced
alignment, TitaNet identity, and final source reconstruction on the common
clock. The frozen T191 posterior must first pass exact mechanical provenance
against both FR49 artifacts or be recaptured twice from the same inputs.

The display-only capture, both T226 chronological readings, and both T227
reverse readings are complete and manually reconciled without disagreement.
Because the historical T191 TOML hash
differs, two current-TOML exact-PCM v2.1 executions were retained; their raw
outputs are byte-identical and independently reproduce all 1,348 FR49 A/B
primary producer runs mechanically. Each deterministic A/B tree now contains
33 complete context directories and all required pipeline evidence. The next
T228 then manually authorizes one frozen, false-by-default experiment for the
right-bounded single-codepoint short-primary topology shared by `ref-0327` and
`ref-0417`. T229 is next: wire its TOML-only boolean, implement the exact
structural guards, and add mechanical contract tests before any replay.

Only those four human readings may identify the earliest evidence loss or
choose a next branch. A reusable final-fusion topology requires at least two
independent material contexts and explicit accepted controls; source-absent
cases instead require a separately specified upstream ASR/VAD/alignment
contract before the final speaker seal. FR50 authorizes no new audio run,
ledger, or closure change before the frozen candidate review. The near-limit
`29.015/28.820 s` terminal waits stay in a separate operational gate and must
not enter speaker correctness.

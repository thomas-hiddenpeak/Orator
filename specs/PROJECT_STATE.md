...
- **Last updated**: 2026-08-09 (Spec 014 Phase 3F one-second accumulated-audio
  candidate rejected by complete contextual review; `kv_append` restored;
  Phase 3G is limited to decoder-state root-cause evidence)
- **Branch**: `master`
- **Constitution**: v1.7.0
...
| Native Qwen3-ASR engine | One-second mismatch demonstrated; full-context and 800-frame corrections rejected; 100-frame control restored | Stored stage fixtures report mel 3.9e-3, encoder 1.3e-3, and decoder argmax parity for their frozen fixtures. Spec 014 T061 pins official source `7c6daf7` and repairs stale oracle provenance. T062 demonstrates that tested 100-frame standalone appends differ from their full-window slices, while complete 800-frame attention windows match exactly. Complete focused contextual review rejects full-context Final replacement because it creates new critical assertions. Phase 3D then adds typed `[asr].stream_window_mel_frames`, strictly limited to evidenced values 100/800. Clean commit `291c63d` tests only the 800-frame value on the same 102-second real-WebSocket context. Direct forward/reverse and comprehensive-view review finds useful lexical repairs, but critical long-segment truncation, invented numeric meaning, repeated short-tail business errors, and unusable Live latency. The candidate is rejected and TOML returns to 100; parameterization remains diagnostic evidence. A current full real-WebSocket baseline remains below ASR closing gates. Active-speech microphone, endpoint, candidate duration ladder, full A/B, and holdout acceptance remain open. Pure bf16 compute. |
...
| Test suite | 75 configured CTest entries | The active Sortformer gates are bound to v2.1. Focused C++ tests cover typed speaker evidence, capture-ordered speaker-identity replay input, ASR full-context Final PCM/replacement/retraction, strict 100/800-frame streaming-window configuration, typed ASR stream modes and rollback values, embedding-cache lifecycle, bounded precompute, final draining, accepted production fusion/abstention policy, and monotonic telemetry deadlines; active Python tests cover ASR oracle provenance, source/time/config invariants, immutable raw tracks, WebSocket manifests, telemetry, evidence integrity, replay inputs, and unjudged review packets. Thirty-three historical candidate tests are archived outside CTest. The inactive accumulated-audio implementation under restored `kv_append` passes focused `test_config` and all `75/75` entries in `52.75 s`; the build emits no warning or error diagnostic. Browser and physical-microphone acceptance remain outside CTest. No automated result may assign correctness, aggregate accuracy, rank/select a candidate, or produce product acceptance. |
...
Start Spec 014 T080 by implementing only the Phase 3G opt-in decoder-state trace
described by FR16. Keep it disabled in checked-in TOML and preserve every
ASR/VAD, Sortformer v2.1, FR50, time-base, alignment, and publication value. Do
not start T048, another parameter candidate, 360/600-second gates, or a full run
unless raw state evidence establishes one specific implementation mismatch and
a new SDD candidate is reviewed first.

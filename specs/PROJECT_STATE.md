...
- **Last updated**: 2026-07-19 (FR40 frozen candidate retained; closure open)
- **Branch**: `master`
- **Constitution**: v1.7.0
...
| Revisable comprehensive timeline (Spec 004) | T111 remains the best frozen comparison; FR29/FR30/FR31 rejected; FR32 real-path and FR33-FR40 frozen repairs retained; closure open | `ComprehensiveTimeline` stores typed diarization, ASR, VAD, alignment, voiceprint, and business tracks and publishes immutable snapshots/typed updates. Its VAD snapshot includes observed, active, and confirmed-silence frontiers used by scheduling-invariant ASR gating. `BusinessSpeakerPipeline` consumes typed `SpeakerEvidenceStage` output and owns orchestration/publication; the internal `SpeakerFusionPolicy` owns rule execution. T135's uniform complete-context reconciliation records T111 at `514/556`, FR29/T123 at `505/556`, and FR30/T133 at `497/556`. FR31's broad primary-return guard is rejected. FR32 repairs `ref-0154`; T142's independent full paths each record `506/556` after complete forward/reverse contextual review. FR33-FR40 then add eight separately diagnosed, deterministic frozen repairs; the current manually reconciled frozen candidate is `514/556`, all complete 600-second natural-turn blocks pass, and no new real-path result is claimed. 朱杰 recall, critical attribution, and confident-wrong attribution still fail. FR16ABO's historical `518/556` predates uniform reconciliation and is not used for ranking. Checked-in VAD threshold is `0.5`. Speaker-time, ASR, and independent holdout remain open. |
...
to `514/556` and the 0-600 block to `87/93`. None of these results is a new
real-WebSocket run; Zhu Jie recall, critical and confident-wrong attribution,
speaker-time sign-off, repeatability, and holdout gates remain open.

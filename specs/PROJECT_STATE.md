...
| Native Qwen3-ASR engine | Numerical oracle verified; Spec 014 Phase 1 seal complete; semantic closure open | Stored stage fixtures report mel 3.9e-3, encoder 1.3e-3, and decoder argmax parity. These numerical gates do not establish full contextual semantic, endpoint, Live/Final, or silence-hallucination acceptance. The clean current-commit seal passes all `74/74` tests and two independent 120-second real-WebSocket captures. Complete forward/reverse contextual review finds critical meaning loss in the Chengdu relationship exchange and Tang's `就当...为准` instruction; it authorizes full-baseline capture, not acceptance. Pure bf16 compute. |
...
| ASR + WS integration | Spec 014 120-second seal repeatable; Live duplicate-partial defect open; full acceptance open | `AuditoryStream` owns one private `PipelineAudioCache` per active producer and uses separate worker threads. One session-owned `TimeBase` is injected into all active stores and workers. Final ASR live emission and its typed sink reuse one `text_id`; partial rejection emits a matching retract, and the terminal ASR track serializes the ID. ASR buffers undecided audio, reads typed stable VAD frontiers from `ComprehensiveTimeline`, applies TOML lead/chunk settings, and drains only after final VAD is frozen; forced alignment consumes finalized ASR records. The Spec 014 A/B seal has byte-identical normalized product tracks and exact final ID/content convergence. It also confirms that unchanged partial text is deduplicated for the typed sink but still emitted repeatedly on WebSocket, causing unnecessary Live renders; this engineering defect is not an accuracy claim. Full silence, endpoint, browser, and contextual acceptance remain open. |

...
- **Last updated**: 2026-08-10 (Spec 014 Phase 3G root-cause review is complete;
  the sole Phase 3H official-termination candidate is active and passes all
  engineering gates before exact contextual evidence)
- **Branch**: `master`
- **Constitution**: v1.7.0
- **Active focused work**: **Spec 014 Phase 1 and Phase 2 are complete.** The
  ...
  Transitional SDD commit `5340cd0` is pushed with the restored control. The
  next checked-in TOML reproduces the exact Phase 3F state at
  `accumulated_redecode`, 1000 ms, two unfixed chunks, five rollback tokens,
  and a 32-token stream budget, then changes only `ban_steps` from `3` to `0`
  relative to that immutable artifact. Trace remains false. The focused config
  test passes, complete CTest passes `75/75` in `51.84` seconds, and the build
  emits no warning or error. No product interpretation follows from those
  engineering gates; the identical 102-second contextual run remains next.
...
| Native Qwen3-ASR engine | Phase 3H official-termination candidate active; contextual evidence open | Stored stage fixtures report mel 3.9e-3, encoder 1.3e-3, and decoder argmax parity for their frozen fixtures. Spec 014 T061 pins official source `7c6daf7` and repairs stale oracle provenance. T062 demonstrates that tested 100-frame standalone appends differ from their full-window slices, while complete 800-frame attention windows match exactly. Complete focused contextual review rejects full-context Final replacement and the isolated 800-frame candidate. Phase 3E/3F reject accumulated re-decode at 2000/1000 ms. Phase 3G captures and completely reads 91 exact states, proving the state transition faithful while identifying native first-three-token EOS suppression absent from official vLLM sampling. Phase 3H now reproduces the exact one-second state with only `ban_steps = 0`; focused config and all `75/75` tests pass and the build is warning-clean. No product result follows until complete 102-second contextual review. Active-speech microphone, duration ladder, full A/B, and holdout acceptance remain open. Pure bf16 compute. |
...
| Native ASR streaming (Specs 003 and 014) | Phase 3H candidate active; engineering gates pass; contextual review next | The existing control keeps persistent KV cache, prefix rollback, and partial publication. Phase 3D rejects an 800-frame append candidate. Phase 3E/3F then reject official growing-audio state at 2000/1000 ms after complete contextual readings. Phase 3G's default-off trace captures all 91 one-second transitions; complete forward/reverse review finds the state machine faithful and isolates native EOS suppression as one concrete official-contract mismatch. Checked-in Phase 3H TOML now reproduces the exact Phase 3F one-second state and changes only `ban_steps` to `0` relative to that artifact; trace remains false. Engineering gates pass, but no ASR or speaker claim is made before the exact 102-second contextual reading. No cadence, rollback, or parameter sweep is authorized. Historical script-derived CER and parameter-sweep claims are not accepted product evidence under Constitution 1.7.0. |
...
| Test suite | 75 configured CTest entries | The active Sortformer gates are bound to v2.1. Focused C++ tests cover typed speaker evidence, capture-ordered speaker-identity replay input, ASR full-context Final PCM/replacement/retraction, strict 100/800-frame streaming-window configuration, typed ASR stream modes/rollback/trace values, exact raw-state JSONL serialization, embedding-cache lifecycle, bounded precompute, final draining, accepted production fusion/abstention policy, and monotonic telemetry deadlines; active Python tests cover ASR oracle provenance, source/time/config invariants, immutable raw tracks, WebSocket manifests, telemetry, evidence integrity, replay inputs, and unjudged review packets. Thirty-three historical candidate tests are archived outside CTest. The active Phase 3H TOML passes focused `test_config` and all `75/75` entries in `51.84 s`; the build emits no warning or error diagnostic. Browser and physical-microphone acceptance remain outside CTest. No automated result may assign correctness, aggregate accuracy, rank/select a candidate, or produce product acceptance. |
...
- [specs/014-asr-vad-closing/spec.md](014-asr-vad-closing/spec.md) - **Live publication is accepted through 600 seconds; Phase 3G root-cause review is complete; the sole Phase 3H decoder candidate passes engineering gates and awaits contextual evidence; ASR semantics and voiced microphone remain open (2026-08-10)**. The focused spec freezes FR50 speaker behavior while closing ASR, VAD endpoint, Live/Final, microphone, and browser evidence. Four prior ASR state hypotheses are rejected by complete contextual review. Phase 3G captures 91 exact accumulated decoder states at clean commit `b92f6ba`; every row is read chronologically and in reverse against the complete human conversation. Accumulation, prefix rollback, and Final state transitions match pinned official source `7c6daf7`. Native first-three-token EOS suppression is absent from official vLLM greedy sampling. Phase 3H now reproduces the exact rejected one-second state with only `ban_steps = 0`; focused config, all `75/75` tests, and a warning-clean build pass. No cadence, rollback, VAD, prompt, alignment, speaker, or fusion change is authorized. Every product conclusion is directly reviewed; scripts never evaluate correctness or choose a candidate. ASR remains below closing requirements and the conditional FR50 speaker boundary remains unchanged. See [decoder-state-root-cause-review-2026-08-10.md](014-asr-vad-closing/decoder-state-root-cause-review-2026-08-10.md). Raw artifacts remain under gitignored `artifacts/spec014/`.
...
Commit and push the engineering-clean Phase 3H candidate, then stream the
identical 102-second WAV at 1x through the real WebSocket with early/late
observers, direct `end`, continuous telemetry, exact provenance, and common
time-base reconciliation. Read every Live, Final, and comprehensive
contribution chronologically and in reverse against complete `test.txt`
context. Do not start T048, another decoder parameter, 360/600-second gates, or
a full run unless this candidate passes that complete contextual review.

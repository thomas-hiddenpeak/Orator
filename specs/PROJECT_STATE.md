...
- **Last updated**: 2026-08-09 (Spec 014 Phase 3F rejected and `kv_append`
  restored; Phase 3G inactive decoder-state trace passes engineering gates and
  awaits exact evidence capture)
- **Branch**: `master`
- **Constitution**: v1.7.0
...
| Native Qwen3-ASR engine | Failed state hypotheses restored; inactive raw-state trace engineered | Stored stage fixtures report mel 3.9e-3, encoder 1.3e-3, and decoder argmax parity for their frozen fixtures. Spec 014 T061 pins official source `7c6daf7` and repairs stale oracle provenance. T062 demonstrates that tested 100-frame standalone appends differ from their full-window slices, while complete 800-frame attention windows match exactly. Complete focused contextual review rejects full-context Final replacement and the isolated 800-frame candidate. Phase 3E/3F then reject official accumulated re-decode at 2000/1000 ms after exact captures and complete contextual readings. TOML restores `kv_append` and 100 frames. Phase 3G adds a default-off JSONL trace that captures raw accumulated decoder state without reference text, scoring, comparison, or product output change. A current full real-WebSocket baseline remains below ASR closing gates. Active-speech microphone, endpoint, candidate duration ladder, full A/B, and holdout acceptance remain open. Pure bf16 compute. |
...
| Native ASR streaming (Specs 003 and 014) | Accumulated candidates rejected; `kv_append` restored; raw trace inactive | The existing control keeps persistent KV cache, prefix rollback, and partial publication. Phase 3D proves that changing only its append unit to a numerically local 800-frame window does not produce acceptable text, so dormant `stream_window_mel_frames` remains 100. Phase 3E implements the official growing-audio state behind typed TOML. Clean `a6ba893` passes engineering gates and the exact 102-second real-WebSocket focus, but complete forward/reverse review rejects its false Final action, exclusive-signing regression, legal substitutions, critical unit loss, and visible investment hallucination. Phase 3F clean commit `244006e` changes only accumulated cadence to 1000 ms and passes engineering/mechanical gates, but complete forward/reverse reading finds the same critical Finals and additional unstable Live text. TOML returns to `kv_append` with dormant 2000 ms settings. Phase 3G's typed JSONL trace is false with an empty path under the checked-in control and permits only exact raw decoder-state evidence; no cadence or rollback sweep is authorized. Historical script-derived CER and parameter-sweep claims are not accepted product evidence under Constitution 1.7.0. |
...
| Test suite | 75 configured CTest entries | The active Sortformer gates are bound to v2.1. Focused C++ tests cover typed speaker evidence, capture-ordered speaker-identity replay input, ASR full-context Final PCM/replacement/retraction, strict 100/800-frame streaming-window configuration, typed ASR stream modes/rollback/trace values, exact raw-state JSONL serialization, embedding-cache lifecycle, bounded precompute, final draining, accepted production fusion/abstention policy, and monotonic telemetry deadlines; active Python tests cover ASR oracle provenance, source/time/config invariants, immutable raw tracks, WebSocket manifests, telemetry, evidence integrity, replay inputs, and unjudged review packets. Thirty-three historical candidate tests are archived outside CTest. The inactive trace under restored `kv_append` passes focused config/JSON/Qwen `3/3` and all `75/75` entries in `52.90 s`; the build emits no warning or error diagnostic. Browser and physical-microphone acceptance remain outside CTest. No automated result may assign correctness, aggregate accuracy, rank/select a candidate, or produce product acceptance. |
...
- [specs/014-asr-vad-closing/spec.md](014-asr-vad-closing/spec.md) - **Baseline and Live publication correction complete through 600 seconds; Chromium file flow and bounded room tone reviewed; four ASR state hypotheses rejected; `kv_append` control restored; ASR semantics and voiced microphone remain open (2026-08-09)**. This focused spec freezes FR50 speaker behavior while establishing full contextual ASR, VAD endpoint, Live/Final, microphone, and browser evidence. The current full baseline has complete chronological and reverse contextual review of all 556 contributions. Commit `a1c8d1d` removes unchanged direct partial publication without changing final product tracks and passes warning-clean build, `74/74` tests, three directly reviewed silence sessions, repeated 120-second, 360-second, and 600-second real-WebSocket gates. Commits `b0eadbe` and `d09b13b` correct browser persistence and source-clock pacing; the clean 120-second Chromium flow passes complete mechanical and contextual review. A real physical-endpoint room-tone session at `d2ac5d1` has no substantive speech assertion, but the only board analog source cannot currently deliver controlled speech, so active microphone scenarios remain open. Complete direct rereading and a common-clock trace of `text_id=133` isolate one prompt-conditioned decoder factor: `语音识别` appears in Live and final ASR before alignment and matches the configured system prompt. Clean commit `5accc5f` tests and rejects an empty-system-prompt candidate. T061/T062 then pin the official implementation and demonstrate that independent 100-frame encoder appends violate the accepted 800-frame locality contract. Clean commit `5b6ba51` tests and rejects complete-segment full-context Final replacement after it creates new critical assertions. Phase 3D exposes `[asr].stream_window_mel_frames`, rejects values outside the evidenced 100/800 pair, and passes both engineering and numerical gates. Clean commit `291c63d` then runs only the 800-frame candidate on the identical 102-second context. Complete chronological, reverse, Live, Final, and comprehensive-view review finds useful lexical repairs but critical truncation, invented numeric meaning, repeated short-tail errors, and unusable Live latency. T067 rejects it, T068 does not authorize dual-path replay, and TOML returns to the 100-frame control. Phase 3E and Phase 3F then test the official accumulated-audio transition at 2000 ms and 1000 ms. Both exact real-WebSocket captures pass engineering and mechanical gates, but complete forward/reverse reading rejects the same critical Final errors and visible hallucinations. TOML returns to `kv_append`; Phase 3G permits only bounded decoder-state root-cause evidence, not another parameter candidate. Every product conclusion is directly reviewed; ASR remains below closing requirements and the conditional FR50 speaker boundary remains unchanged. See [full-baseline-context-review-2026-08-09.md](014-asr-vad-closing/full-baseline-context-review-2026-08-09.md), [live-partial-publication-review-2026-08-09.md](014-asr-vad-closing/live-partial-publication-review-2026-08-09.md), [browser-persistence-review-2026-08-09.md](014-asr-vad-closing/browser-persistence-review-2026-08-09.md), [physical-microphone-review-2026-08-09.md](014-asr-vad-closing/physical-microphone-review-2026-08-09.md), [final-asr-prompt-causality-review-2026-08-09.md](014-asr-vad-closing/final-asr-prompt-causality-review-2026-08-09.md), [streaming-encoder-boundary-review-2026-08-09.md](014-asr-vad-closing/streaming-encoder-boundary-review-2026-08-09.md), [official-accumulated-2s-review-2026-08-09.md](014-asr-vad-closing/official-accumulated-2s-review-2026-08-09.md), and [official-accumulated-1s-review-2026-08-09.md](014-asr-vad-closing/official-accumulated-1s-review-2026-08-09.md). Raw artifacts remain under gitignored `artifacts/spec014/`.
...
Commit and push the inactive Phase 3G trace source, then start Spec 014 T082.
Enable it only in an isolated `orator.toml` for the exact 102-second source and
retain source/config/binary/run/trace hashes plus time-base, observer, and
telemetry evidence. Preserve every ASR/VAD, Sortformer v2.1, FR50, alignment,
time-base, and publication value. Do not start T048, another parameter
candidate, 360/600-second gates, or a full run unless T083 identifies one
specific implementation mismatch and a new SDD candidate is reviewed first.

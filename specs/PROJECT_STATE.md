...
- **Last updated**: 2026-06-30 (Spec 011 observability: offline rerun export with captured gpu_telemetry, validated on rate=1 120 s run)
- **Branch**: `master`
- **Constitution**: v1.4.0
...
- [specs/011-observability/spec.md](011-observability/spec.md) — **Implemented** (2026-06-30): offline [rerun](https://rerun.io) visualization, kept entirely in `tools/` (no runtime third-party dep, Art. I). `tools/verify/py/ws_unified_test.py` captures the runtime's periodic `gpu_telemetry`/cursor WS samples into a `telemetry` array in the run JSON; `tools/observability/timeline_to_rerun.py` keys diarization/comprehensive lanes by the global `speaker_id` (`spk_N`), adds per-pipeline RTF lanes (`gpu/<name>/rtf`, `gpu/<name>/active`) + per-track RTF summaries. Telemetry stays OFF by default (`[telemetry]`/`[telemetry.cursor]` in `orator.toml`), enabled per observability run. Validated on a `rate=1` 120 s stream: 125 gpu_telemetry samples captured, stream_rt 0.964×, exported `.rrd` (4 tracks + 125 gpu samples). Workflow doc `tools/observability/README.md`. Follow-ups: live WS→rerun consumer, full-hour acceptance recording.

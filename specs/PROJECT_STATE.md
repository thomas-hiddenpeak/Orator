...
- **Last updated**: 2026-08-10 (Spec 014 Phase 3G exact trace capture and
  complete forward/reverse state review are complete; one official
  decoder-termination mismatch authorizes the bounded Phase 3H candidate while
  `kv_append` remains checked in)
- **Branch**: `master`
- **Constitution**: v1.7.0
...
| Native Qwen3-ASR engine | Phase 3G root cause complete; one bounded termination candidate authorized | Stored stage fixtures report mel 3.9e-3, encoder 1.3e-3, and decoder argmax parity for their frozen fixtures. Spec 014 T061 pins official source `7c6daf7` and repairs stale oracle provenance. T062 demonstrates that tested 100-frame standalone appends differ from their full-window slices, while complete 800-frame attention windows match exactly. Complete focused contextual review rejects full-context Final replacement and the isolated 800-frame candidate. Phase 3E/3F then reject official accumulated re-decode at 2000/1000 ms after exact captures and complete contextual readings. TOML restores `kv_append` and 100 frames. Phase 3G adds a default-off JSONL trace and captures 91 exact states. Complete forward/reverse reading shows that the accumulated transition matches official source, while source inspection finds native first-three-token EOS suppression absent from official vLLM sampling. Phase 3H authorizes only `ban_steps = 0` against the exact one-second state. A current full real-WebSocket baseline remains below ASR closing gates. Active-speech microphone, endpoint, candidate duration ladder, full A/B, and holdout acceptance remain open. Pure bf16 compute. |
...
| Native ASR streaming (Specs 003 and 014) | Phase 3G audited; `kv_append` retained pending one Phase 3H candidate | The existing control keeps persistent KV cache, prefix rollback, and partial publication. Phase 3D proves that changing only its append unit to a numerically local 800-frame window does not produce acceptable text, so dormant `stream_window_mel_frames` remains 100. Phase 3E implements the official growing-audio state behind typed TOML. Clean `a6ba893` passes engineering gates and the exact 102-second real-WebSocket focus, but complete forward/reverse review rejects its false Final action, exclusive-signing regression, legal substitutions, critical unit loss, and visible investment hallucination. Phase 3F clean commit `244006e` changes only accumulated cadence to 1000 ms and passes engineering/mechanical gates, but complete forward/reverse reading finds the same critical Finals and additional unstable Live text. Phase 3G's default-off trace captures all 91 one-second accumulated transitions; complete forward/reverse review finds the state machine faithful and isolates native EOS suppression as one concrete official-contract mismatch. Checked-in TOML remains `kv_append`, dormant 2000 ms, and `ban_steps = 3` until Phase 3H activation. No cadence, rollback, or parameter sweep is authorized. Historical script-derived CER and parameter-sweep claims are not accepted product evidence under Constitution 1.7.0. |
...
- [specs/014-asr-vad-closing/spec.md](014-asr-vad-closing/spec.md) - **Live publication is accepted through 600 seconds; Phase 3G root-cause review is complete; one Phase 3H decoder candidate is authorized; ASR semantics and voiced microphone remain open (2026-08-10)**. The focused spec freezes FR50 speaker behavior while closing ASR, VAD endpoint, Live/Final, microphone, and browser evidence. Four prior ASR state hypotheses are rejected by complete contextual review. Phase 3G captures 91 exact accumulated decoder states at clean commit `b92f6ba`; every row is read chronologically and in reverse against the complete human conversation. Accumulation, prefix rollback, and Final state transitions match pinned official source `7c6daf7`. Native first-three-token EOS suppression is absent from official vLLM greedy sampling, so Phase 3H authorizes only `ban_steps = 0` against the exact rejected one-second accumulated state. No cadence, rollback, VAD, prompt, alignment, speaker, or fusion change is authorized. Every product conclusion is directly reviewed; scripts never evaluate correctness or choose a candidate. ASR remains below closing requirements and the conditional FR50 speaker boundary remains unchanged. See [decoder-state-root-cause-review-2026-08-10.md](014-asr-vad-closing/decoder-state-root-cause-review-2026-08-10.md). Raw artifacts remain under gitignored `artifacts/spec014/`.
...
Commit and push the completed Phase 3G evidence report and Phase 3H SDD while
the restored control remains checked in. Then reproduce the exact Phase 3F
one-second accumulated state and change only `ban_steps` from `3` to `0`
relative to that immutable artifact. Keep trace false and every VAD, prompt,
segment, model, alignment, Sortformer v2.1, FR50, time-base, and publication
value fixed. Pass engineering gates and push the clean candidate before the
identical 102-second real-WebSocket capture. Do not start T048, another decoder
parameter, 360/600-second gates, or a full run until every Live, Final, and
comprehensive contribution passes complete forward/reverse contextual review.

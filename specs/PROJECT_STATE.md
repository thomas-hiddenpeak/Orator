...
- **Last updated**: 2026-07-10 (Spec 012 speaker-support diagnostics were full-length WebSocket evaluated, then extended with an explicit `speaker_uncertain` business-view field. Follow-up diar tail TOML experiments and NeMo full-length reference check found no accepted low-cost diar parameter recovery; tail attribution remains below closing-grade.)
- **Branch**: `master`
- **Constitution**: v1.5.0
...
time base. The comprehensive layer is a **derived time-alignment view**: it never
mutates pipeline tracks or invents content, but it may split finalized ASR text
at forced-alignment and diarization boundaries for user-facing attribution.
...
- **Diarization** outputs ONLY its own speaker identities + time codes. It never
  attributes text.
- **Comprehensive timeline** aligns the tracks on the common base and derives the
  user-facing attribution view. It can use forced-alignment units for finer
  intra-ASR placement, but it never rewrites ASR text, diarization segments, or
  VAD evidence.
...
| Revisable comprehensive timeline (Spec 004) | ✅ Implemented (core + VAD pipeline + WS conformance) | Native stateful container + derived speaker/text view. Tracks (diarization, asr, vad, align when enabled) each carry data + `source` meta + time codes; every pipeline emits its own WS message and revisions are source-tagged. The comprehensive view projects finalized ASR `text_id` spans through diarization ownership, uses forced-alignment units when present, preserves per-entry resolved `speaker_id`, and keeps ASR final/text boundaries in terminal snapshots. 2026-07-08: fixed historical comprehensive intervals being rewritten by later local-speaker remaps; added TOML-gated `[timeline].align_snap_pause_sec` and `[timeline].align_boundary_split_tolerance_sec` for align-run splitting near diar boundaries. 2026-07-09: comprehensive entries now expose speaker-support diagnostics (`speaker_support`, selected/total diar coverage, max selected-speaker gap, island count) in live revisions and final timeline JSON; thresholds are TOML-gated under `[timeline]` and do not change attribution. Owner invariant: no overlap → "unknown", never borrowed. |
...
| Test suite | ✅ 47/47 tests pass | Clean build under `-Wall -Wextra`, ZERO warnings. `test_kernels` validates GPU kernel numerics. Python integration tests (`py-ws-comprehensive`, `py-ws-real-audio`) run automatically via `test/run_py_test.py`. 2026-07-09 verification for speaker-support diagnostics: `cmake --build build -j`, `./build/test/test_comprehensive_timeline`, `./build/test/test_config`, warning/error grep clean, `cd build && ctest --output-on-failure` → 47/47, `node --check` clean for the touched Web UI modules, and a 30 s real WS `test.mp3` field-propagation check produced final comprehensive entries with support fields (`weak=2`, `strong=9`, `none=1`). 2026-07-10 full-length real WS support-diagnostics evaluation: 3615.0 s audio, 3619.31 s wall, stream RT 0.999x, diar 773 segments, ASR 288 utterances, align 288 groups, 962 comprehensive entries, all entries carried support fields (`strong=690`, `weak=155`, `none=117`). Context-aware speaker-business review rejected this as an accepted accuracy recovery: the diagnostics expose weak ownership, but 3270-3304 s and the 3000-3600 s tail remain below closing-grade speaker attribution. 2026-07-10 `speaker_uncertain` validation: `cmake --build build -j`, `./build/test/test_comprehensive_timeline`, `./build/test/test_config`, `./build/test/test_json`, `cd build && ctest --output-on-failure` → 47/47, JS syntax checks clean, warning/error grep clean, and a 30 s real WS `test.mp3` run produced 12 final comprehensive entries with `speaker_uncertain` present on all entries (`weak/none=true`, `strong=false`, mismatch=0). |
...
- **Spec 012 speaker-business recovery — current active line** (2026-07-09).

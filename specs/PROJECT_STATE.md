...
- **Last updated**: 2026-07-19 (FR31 rejected; FR32 frozen candidate retained)
- **Branch**: `master`
- **Constitution**: v1.7.0
- **Speaker-business closure**: **BEST FROZEN FULL AVERAGE ABOVE FLOOR;
  ...
  Real-WebSocket promotion and every conjunctive closing gate remain open; see
...
| Revisable comprehensive timeline (Spec 004) | T111 is the best frozen comparison; FR29/FR30/FR31 rejected; FR32 frozen candidate retained; closure open | `ComprehensiveTimeline` stores typed diarization, ASR, VAD, alignment, voiceprint, and business tracks and publishes immutable snapshots/typed updates. Its VAD snapshot includes observed, active, and confirmed-silence frontiers used by scheduling-invariant ASR gating. `BusinessSpeakerPipeline` consumes typed `SpeakerEvidenceStage` output and owns orchestration/publication; the internal `SpeakerFusionPolicy` owns rule execution. T135's uniform complete-context reconciliation records T111 at `514/556`, FR29/T123 at `505/556`, and FR30/T133 at `497/556`. FR31's broad primary-return guard is rejected. FR32 requires exact primary-run TitaNet cross-scale corroboration and manually repairs only `ref-0154` in frozen T123 while leaving T111 unchanged; its real-WebSocket ladder is pending. T111 still fails one fixed block, 朱杰 recall, critical attribution, and confident-wrong attribution. FR16ABO's historical `518/556` predates the uniform reconciliation and is not used for ranking. Checked-in VAD threshold is `0.5`. Speaker-time, ASR, and independent holdout remain open. |
...
- [specs/013-industrial-closing-validation/spec.md](013-industrial-closing-validation/spec.md) - **T135 reconciled the full T111/T123/T133 natural-turn ledgers; FR16ABO, both FR28 600-second promotions, FR29 full T123, FR30 full T133/T134, and FR31 remain rejected; FR32 frozen evidence is retained for real-WebSocket promotion; T102/T084 remain open**. Complete forward/reverse contextual review records T111 at `514/556`, T123 at `505/556`, and T133 at `497/556` before FR32. T111 is the best frozen comparison but fails the 3000-3600 block, 朱杰 recall, critical attribution, and confident-wrong attribution. FR32 changes only the frozen T123 `ref-0154` context and leaves frozen T111 unchanged; no full real-path result is yet attributed to it. Commit `d610de36ed13` closes T112 mechanically without changing speaker behavior. Checked-in `vad.threshold` is `0.5`. Speaker-time, per-speaker time, source-time-offset, ASR, browser/microphone, holdout, report-review, and release-tag gates remain open.

## 7. Immediate next step

Keep the streaming v2.1 `340/1/188/188` profile and the corrected T111
`514/556` frozen comparison fixed. It is not an accepted closing result.
...
T136-T141 are complete. FR31 is rejected after deterministic frozen replay and
complete review of every changed T111/T123 context. FR32 is retained on frozen
evidence after changing only T123 `text_id=84`, repairing `ref-0154`, and
leaving T111 unchanged. The next action is T142: freeze the transitional commit,
then run silence, independent 120-second, complete 600-second, and full
empty/frozen-registry real-WebSocket promotion with contextual review at every
accuracy gate. No full-session acceptance claim follows from frozen replay.

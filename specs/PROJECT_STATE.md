...
| Native Qwen3-ASR engine | Numerical oracle verified; Spec 014 baseline and Live publication gates complete; semantic closure open | Stored stage fixtures report mel 3.9e-3, encoder 1.3e-3, and decoder argmax parity. These numerical gates do not establish contextual product accuracy. A current full real-WebSocket baseline has complete chronological and reverse review of all 556 contributions and remains below the ASR closing gates. The bounded Live correction passes engineering, silence, repeated 120-second, 360-second, and 600-second gates without changing final ASR semantics. The Chromium file flow and one physical-endpoint room-tone review pass; active-speech microphone, endpoint, full A/B, and holdout acceptance remain open. Pure bf16 compute. |
...
| Test suite | 74 configured CTest entries | The active Sortformer gates are bound to v2.1. Focused C++ tests cover typed speaker evidence, capture-ordered speaker-identity replay input, embedding-cache lifecycle, bounded precompute, final draining, accepted production fusion/abstention policy, and monotonic telemetry deadlines; active Python tests cover source/time/config invariants, immutable raw tracks, WebSocket manifests, telemetry, evidence integrity, replay inputs, and unjudged review packets. Thirty-three historical candidate tests are archived outside CTest. FR50 covers the right-bounded aligned-unit policy plus raw-frame primary-run coalescing across blocks, inactive and slot boundaries, terminal closure, disabled behavior, live-cycle limits, final drain, and exact cache reuse. The current full CTest run passes all `74/74` entries in `52.80 s`; the final build emits no warning or error diagnostic. Browser and physical-microphone acceptance remain outside CTest. No automated result may assign correctness, aggregate accuracy, rank/select a candidate, or produce product acceptance. |

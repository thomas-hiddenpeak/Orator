...
- **Last updated**: 2026-07-06 (Spec 010 Phase H conservative speaker-identity policy is implemented behind TOML controls; build clean, warning check clean, ctest 47/47; full-length WS candidate completed but was not accepted as an accuracy fix)
- **Branch**: `master`
- **Constitution**: v1.5.0
...
- [specs/010-speaker-id/spec.md](010-speaker-id/spec.md) — **Implemented, with Phase H experiment not accepted as accuracy fix**: speaker identity (TitaNet-Large voiceprint enrollment / re-identification as a post-diarization stage inside the diar pipeline, Art. III). **Phase A complete & committed**: A1 acquire+convert weights → `models/speaker/titanet_large.safetensors` (108 tensors); A2 NeMo oracle (`tools/reference/titanet_oracle.py`, isolated `tools/.venv-nemo`); A3 pure C++/CUDA `model::TitaNetEmbedder` (`include/model/titanet_embedder.h` + `src/model/titanet_embedder.cu`, time-major [T,C]: mel+per_feature → 5-block ContextNet encoder → attentive statistics pooling → 192-d, F32 weights); A4 `test_titanet` validated vs NeMo oracle (**span cosine 1.000000/0.999999/1.000000, cross-span matrix to 4 decimals; ctest 46/46, no warnings**). **Phase B complete & committed**: `pipeline::SpeakerIdentityStage` (clean-segment gate + per-local embed/match/enroll via `SpeakerDatabase` + revisable local→global map), wired into the diar pipeline behind a `DiarizationWorker` segment-processor hook + `[speaker]` config; diar message/track expose a backward-compatible `speaker_id` field. **Phase H current change**: all speaker-id policy thresholds are TOML-controlled, and an opt-in conservative cross-session mode can delay reset-session matching and keep unmatched later-session slots local-only instead of creating or stealing a global id. Verified 2026-07-06 by unit coverage (`test_config`, `test_speaker_id_stage`), warning check, and full ctest 47/47. Full-length WS candidate `/tmp/orator_phaseh_full.json`: 3615 s audio, 3618.185 s wall, stream RT 0.999x, diar 752, ASR 288, VAD 972, 3611 tegrastats samples; context review [local-diar-review-2026-07-06.md](010-speaker-id/local-diar-review-2026-07-06.md) found it reduced spurious late new global ids into local-only labels but did not restore full-session attribution quality.

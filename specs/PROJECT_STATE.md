...
- **Last updated**: 2026-08-09 (Spec 014 Live partial publication correction
  passes through 600 seconds; Phase 4 Chromium file-input gates and one bounded
  physical-endpoint room-tone review pass; active microphone speech is blocked
  by unavailable effective capture hardware; ASR unchanged)
- **Branch**: `master`
- **Constitution**: v1.7.0
...
| Incremental KV-cache ASR streaming (Spec 003) | Implemented; 800-frame causal candidate rejected; 100-frame control restored | Persistent KV cache, prefix rollback, and partial publication are implemented. The retained T011 numerical probe proves standalone locality for complete 800-mel-frame attention windows. T062 demonstrates that independently freezing 100-frame appends is not equivalent: all 16 tested slices differ and the maximum absolute tensor difference is `0.1759`, versus exact equality for both complete control windows. Source defaults remain `max_new_tokens=32`, `unfixed_chunks=2`, and `unfixed_tokens=5`; the stale Spec 003 statement that runtime uses 15 tokens is not code-authoritative. The full-context Final correction is rejected and disabled. Phase 3D exposes a TOML-owned append window and limits it to evidenced 100/800 values. The focused 800-frame run proves that valid encoder locality alone does not yield acceptable text: Live arrives only at trained-window boundaries, short tails repeat critical errors, and long Finals omit critical clauses. Checked-in TOML is restored to 100 while decoder-continuation and tail analysis remain open. Historical script-derived CER and parameter-sweep claims are not accepted product evidence under Constitution 1.7.0. |
...
| Test suite | 75 configured CTest entries | The active Sortformer gates are bound to v2.1. Focused C++ tests cover typed speaker evidence, capture-ordered speaker-identity replay input, ASR full-context Final PCM/replacement/retraction, strict 100/800-frame streaming-window configuration, embedding-cache lifecycle, bounded precompute, final draining, accepted production fusion/abstention policy, and monotonic telemetry deadlines; active Python tests cover ASR oracle provenance, source/time/config invariants, immutable raw tracks, WebSocket manifests, telemetry, evidence integrity, replay inputs, and unjudged review packets. Thirty-three historical candidate tests are archived outside CTest. After the 800-frame candidate is rejected, the restored 100-frame control passes all `75/75` entries in `52.75 s`; the final build emits no warning or error diagnostic. Browser and physical-microphone acceptance remain outside CTest. No automated result may assign correctness, aggregate accuracy, rank/select a candidate, or produce product acceptance. |
...
Resume Spec 014 Phase 3 root-cause analysis with no active candidate. The empty
system prompt changes ambiguous decoder output but fails its complete focused
context and is removed. Reconcile the historical prompt's stabilizing effect
against its demonstrated lexical contamination before specifying a second
reference-free, TOML-owned hypothesis. Preserve every other ASR/VAD, FR50, and
model value; do not start T048, 360/600-second gates, or a full run until a new
candidate passes its own focused complete-context control.

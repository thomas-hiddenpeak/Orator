...
- **Last updated**: 2026-08-10 (Spec 014 Phase 3H official-termination candidate
  is rejected by complete contextual review; `kv_append`, dormant 2000 ms, and
  `ban_steps = 3` are restored and pass all engineering gates)
- **Branch**: `master`
- **Constitution**: v1.7.0
- **Active focused work**: **Spec 014 Phase 1 and Phase 2 are complete.** The
  ...
  investment Live remains visible. Phase 3H is rejected without a longer gate
  or second decoder parameter. Checked-in TOML returns to `kv_append`, dormant
  2000 ms, and `ban_steps = 3`; focused `test_config` and complete CTest pass
  `75/75` in `52.86` seconds, the build emits no warning or error, and no
  capture process remains. See
...
| Native Qwen3-ASR engine | Phase 3H rejected; restored control passes engineering gates | Stored stage fixtures report mel 3.9e-3, encoder 1.3e-3, and decoder argmax parity for their frozen fixtures. Spec 014 T061 pins official source `7c6daf7`. T062 demonstrates that tested 100-frame standalone appends differ from their full-window slices, while complete 800-frame attention windows match exactly. Complete focused contextual review rejects full-context Final replacement and the isolated 800-frame candidate. Phase 3E/3F reject accumulated re-decode at 2000/1000 ms. Phase 3G captures and completely reads 91 exact states, proving the state transition faithful while identifying native first-three-token EOS suppression absent from official vLLM sampling. Phase 3H removes that suppression, but complete reading of 81 Live, eight Final, and 28 comprehensive records rejects it without a critical repair. `kv_append`, dormant 2000 ms, and `ban_steps = 3` are restored. Active-speech microphone, a new independently evidenced ASR hypothesis, duration ladder, full A/B, and holdout acceptance remain open. Pure bf16 compute. |
...
| Native ASR streaming (Specs 003 and 014) | Accumulated branch closed; `kv_append` restored | The control keeps persistent KV cache, prefix rollback, and partial publication. Phase 3D rejects an 800-frame append candidate. Phase 3E/3F reject official growing-audio state at 2000/1000 ms. Phase 3G's default-off trace captures all 91 one-second transitions; complete forward/reverse review finds the state machine faithful and isolates native EOS suppression as one concrete official-contract mismatch. Phase 3H changes only `ban_steps` to `0`, passes mechanical gates, but complete contextual review finds no critical repair and retains the false Live investment assertion. Checked-in TOML restores `kv_append`, dormant 2000 ms, `ban_steps = 3`, and trace false; all `75/75` tests pass. No EOS, cadence, rollback, unfixed-prefix, token-budget, or parameter sweep is authorized. Historical script-derived CER and parameter-sweep claims are not accepted product evidence under Constitution 1.7.0. |
...
- [specs/014-asr-vad-closing/spec.md](014-asr-vad-closing/spec.md) - **Live publication is accepted through 600 seconds; Phase 3G root-cause review is complete; Phase 3H is rejected and the accumulated decoder branch is closed; ASR semantics and voiced microphone remain open (2026-08-10)**. The focused spec freezes FR50 speaker behavior while closing ASR, VAD endpoint, Live/Final, microphone, and browser evidence. Phase 3G captures and completely reads 91 exact accumulated states, establishing a faithful state machine and one official EOS-suppression mismatch. Clean Phase 3H commit `a9ebea7` removes that suppression and passes all mechanical gates. The reviewer then reads every one of 81 Live, eight Final, and 28 comprehensive contributions chronologically and in reverse against complete `test.txt` context. The correct Live negation is still overwritten, `独家` remains absent, legal/name/unit substitutions remain, and the false investment Live remains visible. The candidate is rejected without a longer gate or another decoder parameter. TOML restores `kv_append`, dormant 2000 ms, `ban_steps = 3`, and trace false; focused config, all `75/75` tests, and a warning-clean build pass. Every product conclusion is directly reviewed; scripts never evaluate correctness or choose a candidate. ASR remains below closing requirements and the conditional FR50 speaker boundary remains unchanged. See [official-greedy-termination-review-2026-08-10.md](014-asr-vad-closing/official-greedy-termination-review-2026-08-10.md). Raw artifacts remain under gitignored `artifacts/spec014/`.
...
Commit and push the Phase 3H rejection and restored control. Do not start T048,
another accumulated decoder parameter, a 360/600-second candidate gate, or a
full candidate run. The next ASR product change requires a new independently
evidenced model or decoder hypothesis written into Spec 014 before activation;
until then, retain `kv_append`, dormant 2000 ms, `ban_steps = 3`, the current
VAD/prompt/segment values, Sortformer v2.1, and frozen FR50 behavior.

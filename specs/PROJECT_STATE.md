...
- **Last updated**: 2026-08-09 (Spec 014 Live partial publication correction
  passes through 600 seconds; Phase 4 Chromium file input found and corrected
  a session-persistence collision; clean-commit repeat next; ASR unchanged)
- **Branch**: `master`
- **Constitution**: v1.7.0
- **Active focused work**: **Spec 014 Phase 1 and Phase 2 are complete.** The
  ...
  required before T040 completes. This is not a model, endpoint, speaker, or
  accuracy finding. See
...
| Session persistence UI (Spec 004 T135) | Implemented; Spec 014 reset-lifecycle correction awaiting clean repeat | `SessionStore`, sessions/load RPCs, and the Web UI history panel are active. Metadata parsing handles current `audio_sec` plus legacy `audio_duration`. The 2026-08-09 Chromium gate exposed same-second empty-reset overwrite after `End -> Clear`; the bounded correction skips zero-sample persistence and uses microsecond/PID/sequence IDs. Focused tests, `74/74` CTest, and one corrected 12-second browser flow reload the exact terminal document; an exact clean-commit repeat remains open. |
...
- [specs/014-asr-vad-closing/spec.md](014-asr-vad-closing/spec.md) - **Baseline review and bounded Live publication correction complete through 600 seconds; browser/microphone next (2026-08-09)**. This focused spec freezes FR50 speaker behavior while establishing full contextual ASR, VAD endpoint, Live/Final, microphone, and browser evidence. The current full baseline has complete chronological and reverse contextual review of all 556 contributions. Commit `a1c8d1d` removes unchanged direct partial publication without changing final product tracks and passes warning-clean build, `74/74` tests, three directly reviewed silence sessions, repeated 120-second, 360-second, and 600-second real-WebSocket gates. Every in-scope reference contribution is read in both directions; ASR remains below closing requirements and the conditional FR50 speaker boundary remains unchanged. See [full-baseline-context-review-2026-08-09.md](014-asr-vad-closing/full-baseline-context-review-2026-08-09.md) and [live-partial-publication-review-2026-08-09.md](014-asr-vad-closing/live-partial-publication-review-2026-08-09.md). Raw artifacts remain under gitignored `artifacts/spec014/`; product judgments remain context-only.

## 7. Immediate next step

Execute Spec 014 Phase 4 against clean commit `a1c8d1d`: validate file-input
Live/retract/final replacement, reconnect, persistence, reload, export, and
desktop/mobile rendering in a real browser, then run and contextually review
physical-microphone silence, room tone, short speech, continuous speech, pause,
interruption, overlap, and ordinary background-noise sessions. Automation is
limited to browser and transport mechanics; it cannot judge transcript,
endpoint, hallucination, or speaker correctness.
...
open because ASR and the 19 critical speaker residuals are conjunctive
requirements.

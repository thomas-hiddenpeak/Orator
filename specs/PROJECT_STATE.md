...
- **Last updated**: 2026-07-23 (FR50 freezes the exact FR49 A/B evidence and
  authorizes a display-only, all-residual earliest-evidence-loss audit; no
  production policy, TOML, ledger, or closure status changes)
- **Branch**: `master`
- **Constitution**: v1.7.0
...
The evidence tool is display-only. Complete chronological and reverse
contextual semantic reading of every A and B context against `test.txt` is the
only permitted way to identify the earliest evidence loss or choose a next
branch. A reusable final-fusion topology requires at least two independent
material contexts and explicit accepted controls; source-absent cases instead
require a separately specified upstream ASR/VAD/alignment contract before the
final speaker seal. FR50 authorizes no production policy, TOML, new audio run,
ledger, or closure change. The near-limit `29.015/28.820 s` terminal waits stay
in a separate operational gate and must not enter speaker correctness.

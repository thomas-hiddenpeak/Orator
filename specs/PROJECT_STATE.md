...
- **Last updated**: 2026-08-10 (Constitution 1.8.0 supersedes the focused
  Phase 3H product rejection; its 102-second findings are diagnostic only and
  the exact candidate is scheduled for complete full-length evaluation)
- **Branch**: `master`
- **Constitution**: v1.8.0
- **Active focused work**: **Spec 014 Phase 1 and Phase 2 are complete.** The
  ...
  prohibition, and accumulated-branch closure are superseded. The checked-in
  TOML currently remains at the restored `kv_append`, dormant 2000 ms, and
  `ban_steps = 3` control; focused `test_config` and complete CTest pass
  `75/75` in `52.86` seconds, the build emits no warning or error, and no
  capture process remains. Phase 3I will reactivate the exact candidate and
  requires a 3615.120-second real-WebSocket run plus complete chronological and
  reverse contextual review of all 556 `test.txt` contributions before any
  product verdict. See
...
| Native Qwen3-ASR engine | Phase 3H full-length re-evaluation pending; restored control currently checked in | Stored stage fixtures report mel 3.9e-3, encoder 1.3e-3, and decoder argmax parity for their frozen fixtures. Spec 014 T061 pins official source `7c6daf7`. T062 demonstrates that tested 100-frame standalone appends differ from their full-window slices, while complete 800-frame attention windows match exactly. Phase 3G captures and completely reads 91 exact states, proving the accumulated transition faithful while identifying native first-three-token EOS suppression absent from official vLLM sampling. Phase 3H removes that suppression; its 102-second reading finds unresolved local defects but has no product-decision authority under Constitution 1.8.0. `kv_append`, dormant 2000 ms, and `ban_steps = 3` remain checked in only until Phase 3I reactivates the exact candidate for full `test.mp3` review. Full semantic, active-speech microphone, full A/B, and holdout acceptance remain open. Pure bf16 compute. |
...
| Native ASR streaming (Specs 003 and 014) | Phase 3H full-length re-evaluation authorized; `kv_append` temporarily restored | The control keeps persistent KV cache, prefix rollback, and partial publication. Phase 3G's default-off trace captures all 91 one-second transitions; complete forward/reverse diagnostic review finds the state machine faithful and isolates native EOS suppression as one concrete official-contract mismatch. Phase 3H changes only `ban_steps` to `0` against the one-second accumulated state and passes mechanical gates. Its 102-second review finds unresolved local defects but cannot reject the candidate, state an overall regression, or close the branch under Constitution 1.8.0. Checked-in TOML temporarily remains `kv_append`, dormant 2000 ms, `ban_steps = 3`, and trace false; all `75/75` tests pass. Phase 3I must reactivate the exact candidate and complete the full canonical run and dual contextual review before a product verdict. No second decoder parameter is authorized. Historical script-derived CER and parameter-sweep claims are not accepted product evidence. |
...
- [specs/014-asr-vad-closing/spec.md](014-asr-vad-closing/spec.md) - **Live publication is accepted through bounded engineering levels; Phase 3G root-cause review is complete; Phase 3H product rejection is superseded and Phase 3I full-length evaluation is active; ASR semantics and voiced microphone remain open (2026-08-10)**. Phase 3G captures and reads 91 exact accumulated states, establishing a faithful state machine and one official EOS-suppression mismatch. Clean Phase 3H commit `a9ebea7` removes that suppression and passes all mechanical gates. Its complete local reading of 81 Live, eight Final, and 28 comprehensive contributions identifies an overwritten negation, absent `独家`, legal/name/unit substitutions, and a false investment Live. Constitution 1.8.0 classifies the 102-second evidence as diagnostic only; it cannot reject the candidate, state global accuracy, or stop full evaluation. The exact `accumulated_redecode`, 1000 ms, `ban_steps = 0` candidate must now process complete `test.mp3` and receive complete chronological/reverse contextual review of all 556 `test.txt` contributions before any product decision. Raw artifacts remain under gitignored `artifacts/spec014/`.
...
Execute Spec 014 Phase 3I. Reactivate the exact Phase 3H candidate, pass and
push its engineering gates, then run all 3615.120 seconds of `test.mp3` through
the production WebSocket at 1.0x. Review all 556 `test.txt` contributions and
all relevant raw pipeline evidence chronologically and in reverse context
before stating global accuracy, overall change, or a candidate verdict. Keep
the current VAD/prompt/segment values, Sortformer v2.1, and frozen FR50 behavior
unchanged. Do not test a second decoder parameter.

...
- **Last updated**: 2026-08-09 (Spec 014 Phase 3E two-second accumulated-audio
  candidate rejected by complete contextual review; `kv_append` restored;
  Phase 3F one-second cadence candidate specified but not active)
- **Branch**: `master`
- **Constitution**: v1.7.0
...
| Native ASR streaming (Specs 003 and 014) | Two-second accumulated candidate rejected; KV control restored; one-second cadence specified | The existing control keeps persistent KV cache, prefix rollback, and partial publication. Phase 3D proves that changing only its append unit to a numerically local 800-frame window does not produce acceptable text, so checked-in `stream_window_mel_frames` remains 100. Phase 3E implements the official growing-audio state behind typed TOML. Clean `a6ba893` passes engineering gates and the exact 102-second real-WebSocket focus. Complete forward/reverse review finds materially better long continuation but rejects false Final action, exclusive-signing regression, legal substitutions, critical unit loss, and a visible investment hallucination. Checked-in `stream_mode` returns to `kv_append`. Phase 3F authorizes one 1000 ms accumulated cadence because the rejected short Final occurred before the two-unfixed-chunk boundary; no sweep or product promotion is authorized. Historical script-derived CER and parameter-sweep claims are not accepted product evidence under Constitution 1.7.0. |
...
| Test suite | 75 configured CTest entries | The active Sortformer gates are bound to v2.1. Focused C++ tests cover typed speaker evidence, capture-ordered speaker-identity replay input, ASR full-context Final PCM/replacement/retraction, strict 100/800-frame streaming-window configuration, typed ASR stream modes and rollback values, embedding-cache lifecycle, bounded precompute, final draining, accepted production fusion/abstention policy, and monotonic telemetry deadlines; active Python tests cover ASR oracle provenance, source/time/config invariants, immutable raw tracks, WebSocket manifests, telemetry, evidence integrity, replay inputs, and unjudged review packets. Thirty-three historical candidate tests are archived outside CTest. The inactive accumulated-audio implementation under the restored control passes all `75/75` entries in `53.14 s`; a subsequent clean-first build emits no warning or error diagnostic. Browser and physical-microphone acceptance remain outside CTest. No automated result may assign correctness, aggregate accuracy, rank/select a candidate, or produce product acceptance. |
...
Complete Spec 014 T075 by testing, committing, and pushing the restored
`kv_append` control plus the Phase 3E rejection record. Then activate the single
Phase 3F candidate with `accumulated_redecode` and `stream_chunk_ms = 1000`,
repeat engineering gates, commit the clean source, and stream the identical
102-second focus at 1.0x with 100 ms frames. Read every Live, Final, and
comprehensive-view contribution chronologically and in reverse. Preserve every
other ASR/VAD, FR50, model, time-base, and publication value; do not start T048,
360/600-second gates, or a full run until the focused candidate passes complete
contextual review.

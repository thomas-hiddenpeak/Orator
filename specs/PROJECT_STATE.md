...
- **Last updated**: 2026-07-23 (FR50 T229-T231 implemented the TOML-gated
  right-bounded short-primary unit experiment, completed deterministic frozen
  A/B replay, and retained it through four independent complete contextual
  readings; T232 real-WebSocket promotion is next)
- **Branch**: `master`
- **Constitution**: v1.7.0
- **Speaker-business closure**: **BEST FROZEN REPLAY CANDIDATE `525/556`;
  CURRENT REAL-PATH LEDGER `523/556`; CRITICAL GATES FAIL; FULL CANONICAL
  CLOSURE OPEN**. FR49's
  ...
  contextual regression. The frozen candidate is manually reconciled at
  `525/556`, with 26 confident-wrong, four missing, one uncertain, and 19
  critical residuals. This is not yet a real-path result: no FR50 live
  `test.mp3` run has occurred, and FR49 remains authoritative for production-
  path claims until T232 completes.
...
| Revisable comprehensive timeline (Spec 004) | FR50 frozen replay retained; FR49 remains the current full real-path candidate; closure open | `ComprehensiveTimeline` stores typed diarization, raw diar frame posterior, primary speaker, ASR, VAD, alignment, voiceprint, and business tracks and publishes immutable snapshots/typed updates. Its VAD snapshot includes observed, active, and confirmed-silence frontiers used by scheduling-invariant ASR gating. `BusinessSpeakerPipeline` consumes typed `SpeakerEvidenceStage` output and owns orchestration/publication; the internal `SpeakerFusionPolicy` owns rule execution. T135's uniform complete-context reconciliation records T111 at `514/556`, FR29/T123 at `505/556`, and FR30/T133 at `497/556`. FR31's broad primary-return guard is rejected. FR32-FR47 add separately diagnosed bounded repairs. FR48's fresh speaker-only review and FR49's residual reread establish the corrected pre-candidate real-path ledger at `521/556`. FR49's TOML-gated source-leading primary-prefix policy repairs only `ref-0061` and `ref-0121`; clean commit `1f09052` completes its restarted 120/600/full A/B real-WebSocket ladder. Four complete full-artifact contextual readings manually retain `523/556`, with 27 confident-wrong, five missing, one uncertain, and 20 critical failures. FR50's false-default right-bounded short-primary unit policy is enabled in checked-in TOML. Four complete frozen-candidate readings manually retain `ref-0327` and `ref-0417` at `525/556`, with 26 confident-wrong, four missing, one uncertain, and 19 critical failures. No executable result produces either ledger. FR50 has not run through the live path, so FR49 remains the real-path claim. Checked-in VAD threshold is `0.5`. Speaker-time, ASR, locked holdout, report, and release gates remain open. |
| Reusable common time base (Spec 004) | FR49 full-session A/B mechanical gate passed; product acceptance open | `AuditoryStream` owns one immutable `TimeBase` and injects it into every active private cache, worker, and retained audio store. Finalization reconciles exact sample extents for input, diarization, speaker identity, ASR, VAD, alignment, and business speaker. FR49 full Run A and restarted Run B both close all seven tracks at exactly 57,841,920 samples with zero declared extent gap and true reconciliation flags. This is a mechanical contract result; product closure remains open under Spec 013. |
...
| Streaming validation | Current FR49 full direct-end A/B path passed mechanically | `ws_unified_test.py` has one socket reader, captures source/config/binary pre/post hashes, continuous `tegrastats`, runtime telemetry, and independent terminal-command timing. Acceptance mode sends `end` directly after the final audio frame; explicit `--test-flush` runs record `flush -> end` but are marked ineligible for the 30-second gate. Clean commit `1f09052` passes the restarted 120/600/full A/B ladder. The two exact full streams close all seven tracks at 57,841,920 samples, converge producer/observer terminals, provide complete required telemetry, and finish direct-end at `29.015/28.820 s`. Both pass the 30-second limit with limited margin. Structural checks never evaluate correctness or issue a product verdict; the separate complete contextual reviews are recorded under Spec 013. |
...
| Test suite | 72 configured CTest entries | The active Sortformer gates are bound to v2.1. Focused C++ tests cover typed speaker evidence, capture-ordered speaker-identity replay input, embedding-cache lifecycle, bounded precompute, final draining, accepted production fusion/abstention policy, and monotonic telemetry deadlines; active Python tests cover source/time/config invariants, immutable raw tracks, WebSocket manifests, telemetry, evidence integrity, replay inputs, and unjudged review packets. Thirty-three historical candidate tests are archived outside CTest. FR49 adds a reference-free short-primary evidence exporter regression plus focused primary-run production and bounded fusion abstentions. FR50 adds positive, zero-duration, source-preservation, configuration, and explicit-abstention contracts for the right-bounded aligned-unit policy. The final clean build emits no warning or error diagnostic, and all `72/72` CTest entries pass in `53.56 s`. Browser and physical-microphone acceptance remain outside CTest. No automated result may assign correctness, aggregate accuracy, rank/select a candidate, or produce product acceptance. |
...
- [specs/013-industrial-closing-validation/spec.md](013-industrial-closing-validation/spec.md) - **FR50 T229-T231 right-bounded short-primary unit implementation, deterministic frozen A/B replay, and four independent complete candidate readings retained; T232 real-WebSocket promotion is next; FR49 remains the real-path candidate; speaker-business closure and T102/T084 remain open**. Earlier FR16ABO, FR28, FR29, FR30, and FR31 promotions remain rejected; FR32-FR47 are retained bounded historical repairs and FR46 is evidence-only. FR48's complete speaker-only reread and FR49's residual inventory establish the corrected pre-candidate real-path ledger at `521/556`. FR49's exact topology repairs `ref-0061` and `ref-0121`. Clean pushed commit `1f09052` then completes two 120-second runs, one 600-second run, and independent full empty/frozen-registry A/B captures. Every full artifact is read completely in chronological and reverse fixed-window context. The four readings manually retain the current real-path ledger at `523/556`, with 27 confident-wrong, five missing, one uncertain, and 20 critical failures. FR50's current-config v2.1 evidence audit manually authorizes a false-default, TOML-only topology shared by `ref-0327` and `ref-0417`. T229 implements its exact structural guards without a new numeric parameter. T230's full frozen A/B outputs and repeats are byte-identical. T231 reads each candidate completely in chronological and reverse fixed windows and manually retains both repairs with no new contextual regression, yielding a frozen-only candidate ledger of `525/556`, with 26 confident-wrong, four missing, one uncertain, and 19 critical failures. No code evaluates correctness, calculates either ledger, or produces the product verdict. No FR50 live `test.mp3` run has occurred. Any upstream ASR/VAD/alignment or speaker-evidence producer change invalidates the affected judgments and requires a fresh exact capture plus complete contextual review. Checked-in `vad.threshold` is `0.5`. Speaker-time, per-speaker time, source-time-offset, ASR, browser/microphone, locked holdout, report-review, and release-tag gates remain open.

## 7. Immediate next step

Keep the streaming v2.1 `340/1/188/188` profile and the exact T229 source,
checked-in TOML, audio, reference, and registry sequence fixed. FR50's
false-default right-bounded short-primary unit policy is implemented and
enabled only in TOML. Its frozen full A/B outputs and independent repeats are
byte-identical. Four complete candidate readings, A/B chronological and A/B
reverse fixed windows, manually retain `ref-0327` and `ref-0417` without a new
contextual regression. The frozen candidate is `525/556`; FR49 remains the
`523/556` real-path baseline.

T232 is next. First commit and push the exact warning-clean candidate. Then run
the real production-WebSocket ladder against `test.mp3`: two independent
120-second captures, one 600-second capture, full empty-registry Run A, and a
server-restarted full Run B using only A's frozen registry. Every gate must use
the same commit, binary, checked-in TOML, audio, and behavioral environment,
with 100 ms frames, `1.0x` pacing, direct `end`, early/late observers, complete
runtime/device telemetry, and exact common-clock reconciliation.

At each stage, automation may capture and verify mechanical contracts only.
The complete in-scope conversation must be read against `test.txt`
chronologically and again in reverse fixed windows before promotion. The full
A and B artifacts each require an independent complete 556-context reading.
Any producer divergence, contextual regression, registry-path failure, or
mechanical gate failure stops the ladder. The near-limit FR49
`29.015/28.820 s` terminal waits remain a separate operational risk and do not
enter speaker correctness.

...
| Native Qwen3-ASR engine | One-second streaming encoder mismatch demonstrated; first bounded Final correction rejected and disabled | Stored stage fixtures report mel 3.9e-3, encoder 1.3e-3, and decoder argmax parity for their frozen fixtures. Spec 014 T061 pins official source `7c6daf7` and repairs the stale oracle paths/provenance. T062's same-mel numerical control gives exact equality for complete 800-frame attention windows, but all tested production 100-frame standalone appends differ from their full-window slices, reaching maximum absolute difference `0.1759`. This is an implementation finding, not a product-accuracy result. Candidate `asr-final-full-context-decode` passes its warning-clean build and all `75/75` tests, then fails complete forward/reverse contextual review of the exact 102-second focused passage: it makes useful Final repairs but introduces new critical assertions about a named participant and the signed agreement type. Its checked-in TOML switch is false and the implementation is inactive evidence only. Phase 3D next isolates the model-defined 800-frame trained window while retaining streaming decoder behavior. No product output is accepted. A current full real-WebSocket baseline remains below ASR closing gates; the empty-system-prompt candidate remains rejected and removed. Active-speech microphone, endpoint, candidate duration ladder, full A/B, and holdout acceptance remain open. Pure bf16 compute. |
...
| Incremental KV-cache ASR streaming (Spec 003) | Implemented; 100-frame append violates the original locality contract | Persistent KV cache, prefix rollback, and one-second partial publication are implemented. The retained T011 numerical probe proves standalone locality for complete 800-mel-frame attention windows. T062 extends that same control and demonstrates that independently freezing 100-frame appends is not equivalent: all 16 tested slices differ and the maximum absolute tensor difference is `0.1759`, versus exact equality for both complete control windows. Production source defaults remain `max_new_tokens=32`, `unfixed_chunks=2`, and `unfixed_tokens=5`; the stale Spec 003 statement that runtime uses 15 tokens is not code-authoritative. The full-context Final correction is rejected and disabled after direct contextual review. Spec 014 Phase 3D next exposes a TOML-owned append window and tests the already verified 800-frame model boundary without changing decoder rollback or Final semantics. Historical script-derived CER and parameter-sweep claims are not accepted product evidence under Constitution 1.7.0. |
...
| Test suite | 75 configured CTest entries | The active Sortformer gates are bound to v2.1. Focused C++ tests cover typed speaker evidence, capture-ordered speaker-identity replay input, ASR full-context Final PCM/replacement/retraction, embedding-cache lifecycle, bounded precompute, final draining, accepted production fusion/abstention policy, and monotonic telemetry deadlines; active Python tests cover ASR oracle provenance, source/time/config invariants, immutable raw tracks, WebSocket manifests, telemetry, evidence integrity, replay inputs, and unjudged review packets. Thirty-three historical candidate tests are archived outside CTest. The restored false-default full-context candidate configuration passes all `75/75` entries in `53.18 s`; the final build emits no warning or error diagnostic. Browser and physical-microphone acceptance remain outside CTest. No automated result may assign correctness, aggregate accuracy, rank/select a candidate, or produce product acceptance. |
...
- [specs/014-asr-vad-closing/spec.md](014-asr-vad-closing/spec.md) - **Baseline and Live publication correction complete through 600 seconds; Chromium file flow and bounded room tone reviewed; two bounded Final candidates rejected; trained-window causal control next; ASR semantics and voiced microphone remain open (2026-08-09)**. This focused spec freezes FR50 speaker behavior while establishing full contextual ASR, VAD endpoint, Live/Final, microphone, and browser evidence. The current full baseline has complete chronological and reverse contextual review of all 556 contributions. Commit `a1c8d1d` removes unchanged direct partial publication without changing final product tracks and passes warning-clean build, `74/74` tests, three directly reviewed silence sessions, repeated 120-second, 360-second, and 600-second real-WebSocket gates. Commits `b0eadbe` and `d09b13b` correct browser persistence and source-clock pacing; the clean 120-second Chromium flow passes complete mechanical and contextual review. A real physical-endpoint room-tone session at `d2ac5d1` has no substantive speech assertion, but the only board analog source cannot currently deliver controlled speech, so active microphone scenarios remain open. Complete direct rereading and a common-clock trace of `text_id=133` isolate one prompt-conditioned decoder factor: `语音识别` appears in Live and final ASR before alignment and matches the configured system prompt. Clean commit `5accc5f` tests and rejects an empty-system-prompt candidate. T061/T062 then pin the official implementation and demonstrate that independent 100-frame encoder appends violate the accepted 800-frame locality contract. Clean commit `5b6ba51` tests complete-segment full-context Final replacement in the same 102-second passage. Full chronological and reverse contextual review finds useful repairs but also new critical assertions about “雷总” and the signed agreement type, so T064 rejects and disables it before longer gates. Phase 3D next isolates the model-defined 800-frame streaming window under TOML while keeping decoder rollback and Final policy fixed. Every product conclusion is directly reviewed; ASR remains below closing requirements and the conditional FR50 speaker boundary remains unchanged. See [full-baseline-context-review-2026-08-09.md](014-asr-vad-closing/full-baseline-context-review-2026-08-09.md), [live-partial-publication-review-2026-08-09.md](014-asr-vad-closing/live-partial-publication-review-2026-08-09.md), [browser-persistence-review-2026-08-09.md](014-asr-vad-closing/browser-persistence-review-2026-08-09.md), [physical-microphone-review-2026-08-09.md](014-asr-vad-closing/physical-microphone-review-2026-08-09.md), [final-asr-prompt-causality-review-2026-08-09.md](014-asr-vad-closing/final-asr-prompt-causality-review-2026-08-09.md), and [streaming-encoder-boundary-review-2026-08-09.md](014-asr-vad-closing/streaming-encoder-boundary-review-2026-08-09.md). Raw artifacts remain under gitignored `artifacts/spec014/`.

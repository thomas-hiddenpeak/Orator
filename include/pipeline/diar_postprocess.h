...
//         offset > onset prevents fragmentation by requiring strong evidence to end.
...
// min_dur_on: minimum speech segment duration (default 0.007s, removes noise splatter)
// min_dur_off: minimum silence duration (default 0.15s, merges brief pauses)
std::vector<core::DiarSegment> OnsetOffsetSegments(
    const core::DiarizationFrames& frames,

...
//
// QoS negotiation: effective QoS = min(publisher_qos,
// subscriber_requested_qos). A subscriber requesting EXACTLY_ONCE cannot get
// better than what the publisher offers, and a publisher at AT_MOST_ONCE cannot
// guarantee more.
...
class TopicRouter {
 public:
  ...
  SubscriptionId Subscribe(const TopicPattern& pattern,
                           ...
                           QoS qos = QoS::AT_MOST_ONCE, bool no_local = false);
  ...
  std::vector<Delivery> Route(const Topic& topic,
